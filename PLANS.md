...
- App-visible `current-thread` continuation should fail closed outside a bound Codex thread by default, with any shell fallback requiring an explicit downgrade flag so the ownership change is intentional and reviewable.
- Shell-launched `attached/current-thread` seeds should also fail closed by default, with explicit `manual-protocol` seeding requiring an intentional flag rather than silently opening a shell-owned attached run.
...
- Read-only foreground entrypoints such as `loop:status` and `--help` should reuse a current `dist/` build instead of forcing a rebuild before every inspection.
