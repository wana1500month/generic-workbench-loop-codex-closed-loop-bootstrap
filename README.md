...
If `packages/loop-orchestrator/dist` is missing after unzip or `SOURCE_ARCHIVE_NOT_CODEX_APP_INSTALL.md` is present, you are not using the installable release ZIP. Source checkouts and source archives should not contain `CODEX_APP_INSTALL.md` or `release-manifest.json`. Prefer installing the generated release ZIP. Run `bash ./init.sh` from a source archive only when the operator explicitly accepts a local `npm ci`/build bootstrap. Skill and npm front-door helpers fail closed by default in this state; set `HARNESS_ALLOW_SOURCE_BOOTSTRAP=1` only when you intentionally want them to invoke bootstrap.
...
- `npm run validate:app-release`: Codex app install ZIP gate; packages the release ZIP and validates release startup without requiring a `codex` binary
- `npm run validate:release-gate`: stricter release gate; proves source/install identity and source-archive reproducibility before validating the install ZIP path
...
- `npm run validate:source-release-identity`: proves source checkouts and release ZIPs cannot be confused by marker files
