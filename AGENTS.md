...
- In the Codex app, `루프 시작` must not end with a user-visible checkpoint-waiting message. Start with `npm run loop:start:codex -- --json`, then continue on the same thread with `$attached-loop` until the run reaches a human stop, an external block, or a terminal state.
