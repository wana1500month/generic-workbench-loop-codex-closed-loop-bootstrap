...
- The runtime now separates `controller_mode = detached|attached` from `transport_mode = codex-exec|current-thread|app-server`; detached currently binds to `codex-exec`, while attached can run on the stock Codex `current-thread` transport or a live `app-server` transport.
- Attached runs now default to the live `app-server` transport, while `current-thread` is an explicit manual same-thread mode rather than a silent auto-attached fallback.
...
- The repo now ships `npm run loop:ui`, a lightweight runtime dashboard that tails `transport-state.json`, `live-state.json`, `round-phase.json`, and recent App Server events for the active run.
