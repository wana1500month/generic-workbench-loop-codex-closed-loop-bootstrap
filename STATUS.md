...
- Adapter drift is now a first-class surfaced state, but adapter migration is still proposal-level: the harness can stop and reopen contract negotiation with `recontract_adapter`, yet it does not automatically rewrite adapter kernels or accept adapter-hash migrations inside the same run.
