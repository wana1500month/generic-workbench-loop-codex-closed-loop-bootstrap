...
- The npm front door is now split explicitly: `loop:single` is detached/headless, `loop:single:codex` is the Codex-owned current-thread seed, and `loop:single:manual` is the intentional shell `manual-protocol` seed.
