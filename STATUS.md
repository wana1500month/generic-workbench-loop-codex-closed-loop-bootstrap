...
- Adapter drift is now a first-class surfaced state, and generated-local runtime-surface drift can auto-apply inside the same run, but kernel wiring changes, boundary breaks, and external adapter drift still stop at proposal/recontract rather than automatic adapter rewrite.
