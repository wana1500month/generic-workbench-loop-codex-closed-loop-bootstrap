...
- Ambiguous Korean document requests such as direct API document or document authoring prompts now return `ambiguous_document_request` with clarification questions instead of being hard-rejected as non-product or prematurely routed to `product_build`.
...
- `loop.ts` status, active checkpoint metadata, and target-decision derivation has started moving into `loop/status-snapshot.ts`, `loop/active-checkpoint.ts`, and `loop/round-target-decision.ts`, following the same small-module direction as default evaluation policy and scorecard artifact handling.
