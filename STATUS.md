...
- Productization pass is underway: prepare now writes `runtime/readiness-report.*` plus `evaluation-policy.generated.*`, strictness levels 1-5 drive target score and custom dimension floors, required custom dimensions gate `target_reached` through per-round `scorecard.json` / `scorecard.md`, adaptive intake infers project kind/evidence surfaces, and non-web targets such as CLI/file/test no longer require browser URLs. CLI/parser/converter/analyzer requests now stay in `product_build`, CLI requests resolve to the `cli-tool` target family, other command-first non-web builds resolve to `command-artifact`, explicit verification/evidence surfaces override family fallback, command-first adapter plans omit browser ready URLs, and default scorecard policy is generated for rounds that start without an intake policy.
