...
- Korean product detection now treats generator/builder/template/automation wording such as `문서 산출물 생성기`, `Markdown 보고서 산출물 생성기`, `보고서 생성기`, `문서 템플릿`, `설치 가이드 템플릿`, `데이터 파이프라인 생성기`, and `데이터 파이프라인 빌더` as `product_build`, while document summary/analysis/translation/review/write requests stay non-product.
- `loop.ts` status and target-decision derivation has started moving into `loop/status-snapshot.ts` and `loop/round-target-decision.ts`, following the same small-module direction as default evaluation policy and scorecard artifact handling.
