...
- Generated-local runtime-surface adapter drift can now auto-apply inside the recontract round: the runtime emits `adapter-migration-proposal.json`, rewrites `.generated/codex-adapter/runtime-config.json` plus `adapter.generated.json`, records `adapter-migration-applied.json`, and authorizes the resulting adapter-hash change through `resume-migration.json` before the same run continues.
