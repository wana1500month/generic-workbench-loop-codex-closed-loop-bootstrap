...
- The runtime now separates `controller_mode = detached|attached`; detached mode keeps the crash-safe supervisor path, while attached mode skips nested Codex planner/negotiation/eval lifts and blocks bootstrap `apply_change` from spawning nested `codex exec`.
