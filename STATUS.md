...
- Operator-surface and transport-state now distinguish `foreground-thread` from `manual-protocol` using real thread binding such as `CODEX_THREAD_ID`, and both artifacts now persist launch origin, surface owner, binding state, entrypoint, and app visibility.
- Current-thread is now the default foreground surface across planner / contract-review / generator-plan / eval / attached-generator stages; each same-thread enhancement now pauses honestly with persisted prompt/response artifacts and resumes from file-backed handoff instead of skipping nested enhancement paths.
- Shell-launched `attached/current-thread` seeds now fail closed unless a real Codex thread binding is present. Starting a shell `manual-protocol` run is still possible, but only with explicit `--allow-manual-protocol-seed`.
...
- App-visible foreground runs now prefer `$attached-loop` as the primary resume surface, omit `resume_command` from `operator-surface.json`, and let `loop:status` treat CLI commands as fallback guidance instead of the default next step.
- App-visible `current-thread` runs now fail closed when a shell tries to `resume` or `phase` them outside a bound Codex thread. Shell continuation is only allowed with explicit `--allow-shell-resume-downgrade`, which makes the downgrade back to `manual-protocol` intentional.
...
- Attached validation now also includes `validate:cli-front-door`, which covers `status`, `phase`, and `resume` across current-thread planning, negotiation, and attached-generator handoffs.
