...
- The runtime now separates `controller_mode = detached|attached`; detached mode keeps the crash-safe supervisor path, while attached mode is the stock Codex current-thread protocol, not an App Server thread/turn container.
- Attached mode now fail-closes every nested `runCodexCommand()` path, including bootstrap `apply_change` and subjective-quality review, instead of relying on per-call-site guards.
