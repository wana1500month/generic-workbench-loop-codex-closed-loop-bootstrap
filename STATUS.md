...
- App Server transport now also supports dedicated attached generator task turns, while current-thread attached generator work uses persisted `attached-generator-prompt.md` and `attached-generator-response.json` artifacts instead of nested child Codex execution.
- Outer-timeout prevention now has a separate supervisor surface through `npm run loop:watch`, which restarts the controller from persisted run state and writes `runtime/supervisor-state.json`.
