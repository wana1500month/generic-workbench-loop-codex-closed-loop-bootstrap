...
- Korean product detection now treats generator/builder/automation wording such as `문서 산출물 생성기`, `Markdown 보고서 산출물 생성기`, `데이터 파이프라인 생성기`, and `설치 가이드 문서 생성기` as `product_build`, while document summary/analysis/translation/review requests stay non-product.
- `loop.ts` status derivation has started moving into `loop/status-snapshot.ts`, following the same small-module direction as default evaluation policy and scorecard artifact handling.
