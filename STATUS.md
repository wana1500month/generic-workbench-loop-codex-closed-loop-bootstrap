...
- Adapter drift is now a first-class surfaced state, generated-local runtime-surface drift can auto-apply inside the same run, and proposal-only migrations now have a human approval lane, but generated-local kernel wiring patches still do not apply in place after approval, and external adapters still stop at proposal/new-run planning rather than true same-run rewrite.
