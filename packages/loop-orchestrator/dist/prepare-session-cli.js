...
const main = async () => {
    ...
    console.log(`Strictness: ${result.evaluationPolicy.strictness_level} / target ${result.evaluationPolicy.target_total_score} / project ${result.evaluationPolicy.project_kind}`);
    console.log(`Readiness: ${result.readiness.status} (${result.readiness.ready ? "ready" : "not ready"})`);
    ...
    if (result.readiness.blockers.length > 0) {
        ...
        for (const blocker of result.readiness.blockers) {
            console.log(`- ${blocker.code}: ${blocker.human_explanation} Fix: ${blocker.how_to_fix}`);
        }
    }
