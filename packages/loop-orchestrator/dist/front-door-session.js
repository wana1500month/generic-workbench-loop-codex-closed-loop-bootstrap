...
const detectLocaleFromText = (value) => /[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]/u.test(value) ? "ko" : "en";
const localeForArtifact = (artifact) => detectLocaleFromText([
    ...
    .filter(Boolean)
    .join("\n"));
const preparationSummaryLinesFromIntake = (intake, locale) => {
    if (locale === "ko") {
        return [
            "\uC900\uBE44\uB41C \uBA85\uC138:",
...
const missingProductFieldsFromSnapshot = (intake) => {
    const missing = [];
...
const productQuestionForField = (field, locale) => {
    if (locale === "ko") {
        switch (field) {
...
const stringifyConflictValue = (value) => {
...
const conflictQuestionFor = (conflict, locale) => {
...
const conflictQuestionsFor = (conflicts, locale) => {
    const questions = conflicts.map((conflict) => conflictQuestionFor(conflict, locale));
    return [...new Set(questions)].slice(0, 3);
};
const buildArtifactResult = (artifact, sessionPath, eventsPath, status) => {
    const locale = localeForArtifact(artifact);
    const adapterPlan = artifact.intake.adapter_plan ??
...
export const getFrontDoorSessionStatus = async (threadId) => {
    const artifact = await loadFrontDoorSessionArtifact(threadId);
    if (!artifact) {
        return undefined;
    }
    const paths = frontDoorSessionPathsForThread(threadId);
    return buildArtifactResult(artifact, paths.session_path, paths.events_path, statusForPhase(artifact.phase));
};
export const runFrontDoorDiscoveryTurn = async (input) => {
    ...
    if (existingSession?.phase === "prepared") {
        ...
        return buildArtifactResult(existingSession, paths.session_path, paths.events_path, "prepared");
    }
    ...
    if (!existingSession && initialResult.status === "ambiguous_document_request") {
        ...
        return buildArtifactResult(artifact, paths.session_path, paths.events_path, "ambiguous_document_request");
    }
    ...
    const snapshotMissingProductFields = resolvedResult.status === "ask_product_questions"
        ? []
        : missingProductFieldsFromSnapshot(mergeResult.intake);
    const readyWithConflicts = snapshotMissingProductFields.length === 0 &&
        resolvedResult.status === "ready_for_prepare" &&
        mergeResult.unresolvedConflicts.length > 0;
    const status = snapshotMissingProductFields.length > 0
    ...
    const missingProductFields = snapshotMissingProductFields.length > 0
        ? snapshotMissingProductFields
        : resolvedResult.missing_product_fields;
    const missingExecutionFields = readyWithConflicts || snapshotMissingProductFields.length > 0
        ? []
        : resolvedResult.missing_execution_fields;
    const missingAdapterFields = readyWithConflicts || snapshotMissingProductFields.length > 0
        ? []
        : resolvedResult.missing_adapter_fields;
    const questions = snapshotMissingProductFields.length > 0
        ? snapshotMissingProductFields
            .slice(0, 3)
    ...
    const questionIds = snapshotMissingProductFields.length > 0
...
//# sourceMappingURL=front-door-session.js.map