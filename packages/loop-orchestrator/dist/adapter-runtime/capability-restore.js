...
import { pathExists, attemptPathForCapability, resultExecutionIdFor, quarantineResultFile, validateAdapterCapabilityResult, verificationProviderForCapability, withExecutionMetadata } from "./shared.js";
export const restoreAdapterCapabilityExecution = async (input) => {
    ...
    const attempt = await loadJson(attemptPath).catch(() => undefined);
...
export const restoreAdapterCapabilityExecutions = async (input) => {
    ...
    const restored = await Promise.all(input.capabilities.map((capability) => restoreAdapterCapabilityExecution({
        loadedAdapter: input.loadedAdapter,
        capability,
        roundDirectory: input.roundDirectory
    })));
    return restored.filter((execution) => Boolean(execution));
};
//# sourceMappingURL=capability-restore.js.map