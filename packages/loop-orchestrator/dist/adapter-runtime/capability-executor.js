...
import { attemptPathForCapability, commandDigestFor, defaultCapabilityResult, execCommand, pathExists, quarantineResultFile, sha256ForBuffer, validateAdapterCapabilityResult, verificationProviderForCapability, withExecutionMetadata } from "./shared.js";
...
const adapterEnvSecretNamePattern = /(^OPENAI_API_KEY$|^CODEX_|^AWS_|^GCP_|^AZURE_|^GITHUB_TOKEN$|^NPM_TOKEN$|(^|_)(AUTH|CREDENTIAL|KEY|PASSWORD|PRIVATE|SECRET|TOKEN)($|_))/i;
const extraInheritedAdapterEnvNames = () => new Set((process.env.HARNESS_ADAPTER_ENV_ALLOWLIST ?? "")
    .split(",")
    .map((name) => name.trim().toUpperCase())
    .filter(Boolean));
const isSensitiveEnvName = (name) => adapterEnvSecretNamePattern.test(name.toUpperCase());
...
const sensitiveEnvValuesForRedaction = (extraEnv) => [
...
export const executeAdapterCapability = async (input) => {
    ...
    const provider = verificationProviderForCapability(input.loadedAdapter.contract, input.capability);
    ...
    if (!capabilitySpec) {
        const result = defaultCapabilityResult(input.capability, `${provider.providerRole === "verifier" ? "Verification provider" : "Adapter"} capability '${input.capability}' is not configured.`);
    ...
    const harnessEnv = {
        ...
        HARNESS_RUNTIME_DIRECTORY: packet.runtime_directory ?? join(packet.run_directory, "runtime"),
        HARNESS_CODEX_SESSION_REGISTRY_PATH: packet.codex_session_registry_path ??
    ...
    if (execution.timedOut) {
        ...
        throw new Error(`Adapter command timed out after ${timeoutMs} ms: ${capabilitySpec.command}`);
    }
    if (execution.outputLimitExceeded) {
        ...
        throw new Error(`Adapter command exceeded output cap (${execution.outputLimitBytes} bytes per stream): ${capabilitySpec.command}`);
    }
    let rawResult;
    let resultRedactionCount = 0;
    if (await pathExists(resultPath)) {
        const loaded = withExecutionMetadata(await loadJson(resultPath), executionId);
