...
import { alignWorkflowChecksToCoreFeatures, adapterPlanMarkdown, buildAdapterPlanFromIntake, defaultVerificationSurfacesForFamily, defaultWorkflowChecksFromCoreFeatures, normalizeVerificationSurfacesForFamily } from "./adapter-plan.js";
import { scaffoldDurableMemoryArtifacts } from "./durable-memory.js";
...
const defaultBootstrapRubricPath = join(repoRoot, "evals", "rubrics", "generic-harness-rubric.json");
const familyHelp = [
...
const slugifyAscii = (value) => value
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 48);
const slugify = (value) => slugifyAscii(value) || "generated-app";
const slugForIndexedFeature = (value, index) => slugifyAscii(value) || `feature-${index + 1}`;
const splitList = (value) => value
    .split(/\r?\n|,/)
    .map((entry) => entry.trim())
    .filter(Boolean);
const uniqueList = (values) => [...new Set(values.filter(Boolean))];
const uniqueAxisList = (axes) => {
    const seen = new Set();
...
const topPreserveSignals = (answers, limit = 4) => uniqueList([
    answers.finishLine,
    ...(answers.mustNotBreak ?? []),
    ...(answers.failureExpectations ?? []),
    ...(answers.qualityBar ?? [])
]).slice(0, limit);
const topReferenceSignals = (answers, limit = 4) => uniqueList([
    ...(answers.referenceSignals ?? []),
    ...(answers.referenceApps ?? [])
]).slice(0, limit);
const nonEmptyProbeHints = (probeHints) => {
    if (!probeHints) {
        return undefined;
    }
    const normalized = Object.fromEntries(Object.entries(probeHints).filter(([, value]) => typeof value === "string" && value.trim()));
    return Object.keys(normalized).length > 0 ? normalized : undefined;
};
const mergeQualityContract = (base, overlay) => ({
...
const mergeSubjectiveMetrics = (baseMetrics, overlayMetrics) => {
...
const buildProductInferenceText = (input) => [
    ...
    .filter(Boolean)
    .join(" ");
const inferGoalLevelFromTargetScore = (targetScore) => {
    for (const [goalLevel, presetScore] of Object.entries(goalPresets)) {
...
export const normalizeBootstrapTargetFamily = (targetFamily) => {
...
const inferPackageManagerFromCommand = (command) => {
...
const normalizeTargetFamily = (value) => {
...
const defaultRootForTitle = (title) => resolve(repoRoot, "..", slugify(title));
...
const defaultHealthUrlForFamily = (targetFamily) => isApiOnlyFamily(targetFamily) ||
    ...
    : undefined;
const defaultApiBaseUrlForFamily = (targetFamily) => isApiOnlyFamily(targetFamily) ||
    targetFamily === "fullstack-app" ||
    targetFamily === "browser-editor" ||
    targetFamily === "dashboard"
...
const toBootstrapWorkflowCheck = (check) => ({
    workflow: check.workflow,
    surface: check.surface,
    ...(check.trigger ? { trigger: check.trigger } : {}),
...
export const buildBootstrapAnswersFromSeed = (seed) => {
    ...
    const checkCommand = seed.checkCommand?.trim() ||
    ...
    const verificationSurfaces = normalizeVerificationSurfacesForFamily(seed.targetFamily, seed.verificationSurfaces);
...
const completeBootstrapAnswers = (answers) => {
    const verificationSurfaces = normalizeVerificationSurfacesForFamily(answers.targetFamily, answers.verificationSurfaces);
...
const apiBackedFamily = (targetFamily) => isApiOnlyFamily(targetFamily) ||
    ...
    targetFamily === "browser-editor" ||
    targetFamily === "dashboard";
const targetSurfacesForFamily = (targetFamily) => uniqueList([
...
const browserSurfaceRequested = (answers) => verificationSurfaceSetFor(answers).has("browser");
const apiSurfaceRequested = (answers) => verificationSurfaceSetFor(answers).has("api");
const targetSurfacesForAnswers = (answers) => uniqueList([
...
const productIntentText = (answers) => [
    answers.title,
    answers.summary,
    answers.finishLine,
    ...
    .join(" ")
    .toLowerCase();
const shouldProbeContinuity = (answers) => {
    const continuityBoundaries = new Set((answers.continuityBoundaries ?? []).map((boundary) => boundary.toLowerCase()));
    if (continuityBoundaries.has("reload") ||
        continuityBoundaries.has("refresh") ||
        continuityBoundaries.has("reopen")) {
        return true;
    }
    return /저장|복원|새로고침|리로드|유지|초안|드래프트|임시저장|autosave|draft|persist|persistence|restore|reload|refresh|reopen|local storage|session storage|state continuity/.test(productIntentText(answers));
};
const shouldProbeErrorRecovery = (answers) => {
    if ((answers.failureExpectations ?? []).length > 0) {
        return true;
    }
    return /오류|에러|검증|실패|거부|잘못된|invalid|validation|reject|recovery|error path|failure/.test(productIntentText(answers));
};
const probeAllowedForSurfaces = (probe, surfaces) => {
...
const probeAllowedForProductIntent = (probe, answers) => {
    const tags = new Set(probe.assertion_tags ?? []);
...
const filterCriteriaForActiveProbes = (criteria, probes) => {
    const activeAssertionIds = new Set(probes
...
const filterVerificationProfileForAnswers = (profile, answers) => {
    const surfaces = verificationSurfaceSetFor(answers);
    const coreProbes = (profile.core_probes ?? []).filter((probe) => probeAllowedForSurfaces(probe, surfaces) &&
    ...
    return {
        ...
        minimum_feature_release_assertions: Math.max(coreProbes.filter((probe) => (probe.role ?? "supporting") === "release_gate")
            .length, 1)
    };
};
const buildGeneratedQualityAxes = (answers) => {
    const workflowNames = (answers.workflowChecks ?? []).length > 0
    ...
    const customMetricAxes = (answers.customQualityMetrics ?? []).map((metric) => ({
        ...
        preserve_signals: topPreserveSignals(answers, 4),
        reference_signals: topReferenceSignals(answers, 4),
        scoring_mode: "subjective_out_of_ten",
            ...
            ? [
                {
                    ...
                    desired_outcome: answers.referenceSignals?.[0] ??
...
const buildGeneratedQualityContract = (answers) => ({
...
const defaultSubjectiveFloorFor = (targetScore) => {
    if (targetScore >= 0.95) {
        return 9;
    }
    if (targetScore >= 0.9) {
...
const defaultSubjectiveMetricsFor = (answers) => {
    ...
    const metrics = [
        ...
        },
        {
            ...
            description: "The product should satisfy the generated adapter workflow checks through real visible behavior, not superficial markers.",
            minimum_score_out_of_ten: floor,
            quality_axis_id: "primary_flow",
            required: true,
            weight: 2
        ...
        },
        {
            ...
            description: "The current build should be materially beyond the initial prototype or scaffold.",
            minimum_score_out_of_ten: Math.max(7, floor - 0.2),
            quality_axis_id: "primary_flow",
...
const customSubjectiveMetricsFor = (answers) => (answers.customQualityMetrics ?? []).map((metric) => ({
...
const buildGeneratedSubjectiveMetrics = (answers) => mergeSubjectiveMetrics(defaultSubjectiveMetricsFor(answers), customSubjectiveMetricsFor(answers));
...
const mergeGeneratedProbeOverlay = (baseProbes, generatedProbes) => {
    const seenProbeIds = new Set();
    const seenAssertionKeys = new Set();
    const merged = [];
...
const mergeAssertionTagCountFloors = (baseCounts, generatedCounts) => {
    const merged = {};
...
const buildGeneratedCriteria = (answers, generatedCoreProbes = buildGeneratedCoreProbes(answers)) => {
    const releaseGateProbeCriteria = generatedCoreProbes
        .filter((probe) => (probe.role ?? "supporting") === "release_gate" && Boolean(probe.assertion_id))
        .flatMap((probe) => [
        {
...
const buildGeneratedCoreProbes = (answers) => {
    ...
    const selectors = {
        appShell: answers.probeHints?.appShellSelector ??
            "[data-harness='app-shell'], [data-testid='app-shell']",
        success: answers.probeHints?.successSelector ??
            "[data-harness='finish-line-ready'], [data-testid='finish-line-ready']",
        error: answers.probeHints?.errorSelector ??
            "[data-harness='error-banner'], [data-testid='error-banner']",
        draftInput: answers.probeHints?.persistenceInputSelector ?? "[data-testid='draft-input']",
        saveAction: answers.probeHints?.saveActionSelector ?? "[data-testid='save-draft']",
        restored: answers.probeHints?.restoredSelector ?? "[data-testid='draft-restored']"
    };
    const apiPaths = {
    ...
    const workflowChecks = (answers.workflowChecks ?? []).length > 0
        ? answers.workflowChecks
        : defaultWorkflowChecksFromCoreFeatures(answers.coreFeatures, requestedVerificationSurfaces).map(toBootstrapWorkflowCheck);
    const featureSlugs = workflowChecks
        ...
        return {
            ...
            check,
            featureSlug,
            axisId: qualityContract.quality_axes.find((axis) => axis.axis_id === axisId)
                ?.axis_id ?? axisId
        };
    });
    const probes = [];
    ...
    if (browserSurfaceRequested(answers)) {
        probes.push(buildBrowserJourneyProbe({
            ...
            .map(({ feature, featureSlug, axisId, check }) => buildBrowserJourneyProbe({
            ...
            steps: [
                ...
                {
                    action: "assert_visible",
                    selector: check.selectorHints?.root ?? `[data-testid='feature-${featureSlug}']`
                },
                {
                    action: "click",
                    selector: check.selectorHints?.action ??
                        `[data-testid='feature-${featureSlug}-action']`
                },
                {
                    action: "assert_visible",
                    selector: check.selectorHints?.result ??
    ...
    if (apiSurfaceRequested(answers)) {
        probes.push(buildApiJsonProbe({
            ...
            .map(({ feature, featureSlug, axisId, check }) => buildApiJsonProbe({
...
const minimumAssertionTagCountsForGeneratedProbes = (probes) => {
    const assertionIdsByTag = {};
    for (const probe of probes) {
        if ((probe.role ?? "supporting") !== "release_gate") {
            continue;
        }
        for (const tag of probe.assertion_tags ?? []) {
            const assertionIds = assertionIdsByTag[tag] ?? new Set();
            assertionIds.add(probe.assertion_id ?? probe.probe_id);
            assertionIdsByTag[tag] = assertionIds;
        }
    }
    return Object.fromEntries(Object.entries(assertionIdsByTag).map(([tag, assertionIds]) => [
        tag,
        assertionIds.size
    ]));
};
const buildGeneratedVerificationProfile = async (answers) => {
    const familySelection = resolveTargetFamilySelection(answers.targetFamily);
    if (!familySelection) {
        throw new Error(`No evaluator bundle is registered for bootstrap target family '${answers.targetFamily}'.`);
    }
    const baseProfile = await loadJson(familySelection.profile_path);
    ...
    const generatedReleaseGateProbeCount = generatedCoreProbes.filter((probe) => (probe.role ?? "supporting") === "release_gate").length;
    const generatedMinimumAssertionTagCounts = minimumAssertionTagCountsForGeneratedProbes(generatedCoreProbes);
    const mergedCoreProbes = mergeGeneratedProbeOverlay(filteredBaseProfile.core_probes ?? [], generatedCoreProbes);
    const mergedCriteria = uniqueCriteria([
        ...(filteredBaseProfile.criteria ?? []),
        ...generatedCriteria
    ]);
    const mergedMinimumAssertionTagCounts = minimumAssertionTagCountsForGeneratedProbes(mergedCoreProbes);
    const titleSlug = slugify(answers.title);
    const qualityContract = mergeQualityContract(filteredBaseProfile.quality_contract, buildGeneratedQualityContract(answers));
    const mergedSubjectiveMetrics = mergeSubjectiveMetrics(filteredBaseProfile.subjective_metrics ?? [], generatedSubjectiveMetrics);
    const mergedScorePolicy = generatedSubjectiveMetrics.length > 0 && browserSurfaceRequested(answers)
        ? {
            ...
            proof_weights: {
                ...
                external_grade: Math.max(filteredBaseProfile.score_policy?.proof_weights?.external_grade ?? 0, 0.6)
            },
            release_weights: {
        ...
        : generatedSubjectiveMetrics.length > 0
            ? {
                ...
                proof_weights: {
                    ...
                    external_grade: Math.max(filteredBaseProfile.score_policy?.proof_weights?.external_grade ?? 0, 0.35)
                }
            }
            : filteredBaseProfile.score_policy;
    ...
    const mergedReleaseGateProbeCount = mergedCoreProbes.filter((probe) => (probe.role ?? "supporting") === "release_gate").length;
    return {
        ...filteredBaseProfile,
        ...
        target_reached_requires_core_probes: filteredBaseProfile.target_reached_requires_core_probes ?? true,
        minimum_feature_release_assertions: Math.max(mergedReleaseGateProbeCount, Math.max(generatedReleaseGateProbeCount, 1)),
        ...
        notes: uniqueList([
            ...
            ...generatedSubjectiveMetrics.map((metric) => `Subjective metric: ${metric.label} >= ${metric.minimum_score_out_of_ten}/10`),
            ...answers.coreFeatures.slice(0, 3).map((feature) => `Core workflow: ${feature}`),
            ...answers.qualityBar.slice(0, 4).map((entry) => `Quality bar: ${entry}`)
        ])
    };
};
const buildGeneratedRubric = async (answers) => {
    const baseRubric = await loadJson(defaultBootstrapRubricPath);
    const qualityContract = buildGeneratedQualityContract(answers);
    return {
        ...baseRubric,
...
const askText = async (rl, label, fallback) => {
    const prompt = fallback ? `${label} [${fallback}]: ` : `${label}: `;
    const answer = (await rl.question(prompt)).trim();
    return answer.length > 0 ? answer : fallback ?? "";
};
const askRequired = async (rl, label, fallback) => {
    while (true) {
...
const askList = async (rl, label, fallback) => {
...
const askOptionalList = async (rl, label, fallback) => {
    const fallbackText = fallback && fallback.length > 0 ? fallback.join(", ") : undefined;
    const answer = await askText(rl, label, fallbackText);
    return splitList(answer);
};
const askYesNo = async (rl, label, fallback = "n") => {
    while (true) {
...
const askScoreOutOfTen = async (rl, label, fallback = 8) => {
    while (true) {
...
const askCustomQualityMetrics = async (rl) => {
    const wantsCustomMetrics = await askYesNo(rl, "Do you want extra quality metrics like design quality or originality? (y/n)", "n");
    if (!wantsCustomMetrics) {
        return [];
    }
    const metrics = [];
    while (true) {
        const label = await askText(rl, "Metric name (leave blank to finish)", metrics.length === 0 ? "design quality" : undefined);
        if (!label.trim()) {
            break;
        }
        const description = await askRequired(rl, `What should '${label}' reward or punish?`);
        const minimumScoreOutOfTen = await askScoreOutOfTen(rl, `Minimum passing score for '${label}' (0-10)`, 8);
        const required = await askYesNo(rl, `Should '${label}' block target_reached if it falls below the threshold? (y/n)`, "y");
        const weight = await askScoreOutOfTen(rl, `Weight for '${label}' in the subjective average (0-10, usually 1 or 2)`, required ? 2 : 1);
        metrics.push({
...
const askProbeHints = async (rl, targetFamily) => {
    const wantsHints = await askYesNo(rl, "Do you want to add probe hints like selectors or API paths? (y/n)", "n");
    if (!wantsHints) {
        return undefined;
    }
    const probeHints = {};
    if (browserBackedFamily(targetFamily)) {
        const appShellSelector = await askText(rl, "App shell selector (leave blank to skip)");
        const successSelector = await askText(rl, "Success selector (leave blank to skip)");
        const errorSelector = await askText(rl, "Error selector (leave blank to skip)");
        const persistenceInputSelector = await askText(rl, "Persistence input selector (leave blank to skip)");
        const saveActionSelector = await askText(rl, "Save action selector (leave blank to skip)");
        const restoredSelector = await askText(rl, "Restored-state selector (leave blank to skip)");
    ...
    if (apiBackedFamily(targetFamily)) {
        const apiFinishLinePath = await askText(rl, "API finish-line path (leave blank to skip)");
        const apiErrorPath = await askText(rl, "API error-path (leave blank to skip)");
        const apiPersistencePath = await askText(rl, "API persistence path (leave blank to skip)");
        Object.assign(probeHints, {
...
const askTargetFamily = async (rl, fallback = "fullstack-app") => {
    while (true) {
...
const askProjectMode = async (rl, fallback = "new") => {
    while (true) {
        const answer = (await askText(rl, "Is this a new project or an existing project? (new/existing)", fallback)).toLowerCase();
...
const askGoalLevel = async (rl, fallback = "usable") => {
    while (true) {
        ...
        output.write("Please choose prototype, mvp, usable, production-like, or custom.\n");
    }
};
const askTargetScore = async (rl, fallback = goalPresets.usable) => {
    while (true) {
        const answer = await askRequired(rl, "Target score (0 to 1)", String(fallback));
        const parsed = Number(answer);
...
const askMaxRounds = async (rl, fallback = 3) => {
    while (true) {
        const answer = await askRequired(rl, "Maximum loop rounds", String(fallback));
        const parsed = Number(answer);
...
const summarizeAnswers = (answers) => [
    ...
    answers.customQualityMetrics?.length
        ? `- Subjective metrics: ${answers.customQualityMetrics
            .map((metric) => `${metric.label} >= ${metric.minimumScoreOutOfTen}/10${metric.required === false ? " (score-only)" : ""}`)
            .join("; ")}`
        : undefined,
    ...
    const lines = [
        ...
        ...(answers.customQualityMetrics?.length
            ? [
                "",
                "## Subjective Metrics",
                ...answers.customQualityMetrics.map((metric) => `- ${metric.label}: ${metric.description} Minimum ${metric.minimumScoreOutOfTen}/10.${metric.required === false ? " Score influence only." : " Blocking if below threshold."}`)
            ]
            : []),
        ...(answers.probeHints && Object.keys(answers.probeHints).length > 0
            ? [
                "",
                "## Probe Hints",
                ...Object.entries(answers.probeHints).map(([key, value]) => `- ${key}: ${value}`)
            ]
            : []),
        "",
        "## Technical Notes",
...
const buildDurableMemoryContextFromAnswers = (answers) => ({
    title: answers.title,
...
export const scaffoldBootstrapArtifacts = async (answers, paths = defaultBootstrapPaths) => {
    ...
    await scaffoldDurableMemoryArtifacts(paths.rootDirectory, buildDurableMemoryContextFromAnswers(answers));
...
const collectAnswers = async () => {
    ...
    try {
        while (true) {
            ...
            const summary = await askRequired(rl, "Describe the product in one or two concrete sentences");
            const targetUsers = await askList(rl, "Who is this for? (comma-separated)", ["founder", "operator"]);
            const coreFeatures = await askList(rl, "What are the core workflows for v1? (comma-separated)", ["primary workflow", "basic persistence"]);
            const referenceApps = await askOptionalList(rl, "Any reference products, screens, or visual directions? (optional)");
            const finishLine = await askRequired(rl, "For the first version, what counts as success?");
            output.write("\nQuality intake\n");
            const mustNotBreak = await askOptionalList(rl, "Which experiences must never break? (optional, comma-separated)", ["draft should survive reload"]);
            const failureExpectations = await askOptionalList(rl, "When the user hits a bad or invalid state, what should they see or be able to do? (optional, comma-separated)", ["show an explicit error message", "offer a retry or recovery path"]);
            const continuityBoundaries = await askOptionalList(rl, "Which boundaries must preserve state? (optional, comma-separated: reload, retry, refresh, reopen)", ["reload"]);
            const referenceSignals = await askOptionalList(rl, "What exactly should feel like the reference? (optional, comma-separated)", ["clear hierarchy", "calm density", "fast transitions"]);
            const nonGoals = await askOptionalList(rl, "What is explicitly out of scope or forbidden? (optional, comma-separated)", ["do not add a settings area"]);
            const customQualityMetrics = await askCustomQualityMetrics(rl);
            const targetFamily = inferProductTargetFamily(buildProductInferenceText({
                title,
                summary,
                targetUsers,
                coreFeatures,
            ...
            const targetRoot = resolveUserPath(await askRequired(rl, "Where should the working project live?", defaultRootForTitle(title)), defaultRootForTitle(title));
            const goalLevel = await askGoalLevel(rl, "usable");
            const targetScore = await askTargetScore(rl, goalLevel === "custom" ? goalPresets.usable : goalPresets[goalLevel]);
            ...
            const runCommand = projectMode === "existing"
                ? await askRequired(rl, "What command should the harness use to start the existing target?", defaultRunCommand)
                : defaultRunCommand;
            ...
            const verificationSurfaces = defaultVerificationSurfacesForFamily(targetFamily);
            const workflowChecks = defaultWorkflowChecksFromCoreFeatures(coreFeatures, verificationSurfaces).map(toBootstrapWorkflowCheck);
            const adapterPlan = buildAdapterPlanFromIntake({
                targetFamily,
                intake: {
                    product_title: title,
...
export const runInteractiveBootstrap = async () => {
