...
export const isBootstrapGeneratedAdapter = (loadedAdapter) => {
    if (!loadedAdapter) {
        return false;
    }
    ...
    const pointsAtGeneratedApplyChange = (value) => value.includes(".generated/codex-adapter/scripts/apply-change") ||
        value.includes("codex-adapter/scripts/apply-change");
    return (loadedAdapter.contract.adapter_id.startsWith("generated-") &&
        (pointsAtGeneratedApplyChange(applyChangeCommand) ||
            applyChangeArgs.some((arg) => pointsAtGeneratedApplyChange(arg))));
};
export const isAttachedGeneratorTransport = (transportMode) => transportMode === "current-thread" || transportMode === "app-server";
const releaseGateProbes = (probes) => (probes ?? []).filter((probe) => (probe.role ?? "supporting") === "release_gate");
const selectorRequirementsFromProbes = (probes) => releaseGateProbes(probes)
    .filter((probe) => probe.mode === "browser_journey")
    .flatMap((probe) => (probe.steps ?? [])
...
const apiRequirementsFromProbes = (probes) => releaseGateProbes(probes)
...
const generatorDeliverablesForPrompt = (input) => input.task.build_brief_snapshot
...
const promptText = (input) => [
    ...
    ...(input.task.build_brief_snapshot
        ? [
            ...
            "- Core workflows:",
            ...input.task.build_brief_snapshot.core_workflows.map((workflow) => `  - ${workflow}`),
            "- Success definition:",
            ...input.task.build_brief_snapshot.success_definition.map((item) => `  - ${item}`)
        ]
        : ["- none"]),
    "",
    "## Required release-gate selectors",
    ...(input.task.verification_requirements?.required_selectors.length
        ? input.task.verification_requirements.required_selectors.map((item) => `- ${item.selector} (${item.action}, probe: ${item.probe_id}, ${item.label})`)
        : ["- none"]),
    "",
    "## Required API release-gate paths",
    ...(input.task.verification_requirements?.api_probe_paths.length
        ? input.task.verification_requirements.api_probe_paths.map((item) => `- ${item.path} => ${item.expected_value ?? "expected value"} (${item.probe_id})`)
        : ["- none"]),
    "",
    ...
    ...(input.previousPatchRequest?.must_fix.length
        ? input.previousPatchRequest.must_fix.map((item) => `- ${item.expected_change} (${item.why})`)
        : ["- none"]),
    "",
    "## Preserve signals",
...
export const writeAttachedGeneratorTask = async (input) => {
    ...
    const task = {
        ...
        must_fix: input.previousPatchRequest?.must_fix.map((item) => item.expected_change) ?? [],
    ...
    await Promise.all([
        ...
        writeText(input.artifacts.attached_generator_prompt_path, promptText({
...
export const readAttachedGeneratorResponse = async (path, expectedCheckpointId) => {
    const parsed = await loadJsonIfExists(path);
...
//# sourceMappingURL=attached-generator.js.map