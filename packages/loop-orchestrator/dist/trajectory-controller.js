...
const isStableRound = (round) => round.threshold_results.dimension_thresholds_met ||
    round.previous_patch_request_resolved ||
    round.overall_verdict === "advance";
const isPassingRound = (round) => round.threshold_results.contract_completed && round.overall_verdict !== "hold";
const isParallelPivotTrigger = (triggerCode) => triggerCode === "plateau_without_progress" ||
    triggerCode === "repeated_same_failure_signature" ||
    triggerCode === "patch_entropy_spike";
const modeForFailurePolicy = (failureLineage, fallback = "pivot") => {
...
const noveltyTargetForMode = (mode) => {
    switch (mode) {
        case "tighten":
            return 0.15;
        case "refine":
...
const anchorReasonForRestart = (input) => {
    if (input.restartFrom === "best_passing") {
        return {
            ...
            anchorReason: input.bestPassingRound !== undefined
    ...
    if (input.restartFrom === "last_stable") {
        return {
            ...
            anchorReason: input.lastStableRound !== undefined
    ...
    return {
        ...
        anchorReason: "Continue from the current head and keep the existing implementation lineage."
    };
};
const reasonForTrajectory = (input) => {
    ...
    if (input.patchRequest.next_action === "recontract_adapter") {
        return (input.patchRequest.adapter_drift_summary ??
...
export const fallbackTrajectoryDirective = (input) => {
    ...
    const discardableSurface = unique(input.previousPatchRequest?.must_fix.map((item) => item.expected_change) ?? []).slice(0, 6);
    let mode = "tighten";
    if (input.remediationHistory?.policy_snapshot?.recommended_action === "recontract") {
        mode = isParallelPivotTrigger(input.remediationHistory.policy_snapshot.dominant_trigger_code)
            ? "parallel_pivot"
            : "pivot";
    }
    ...
    return {
        ...
        discardable_surface: mode === "pivot" || mode === "parallel_pivot" ? discardableSurface : [],
        novelty_target: noveltyTargetForMode(mode),
        reason: input.remediationHistory?.policy_snapshot?.reasons.join(" ") ||
...
export const buildTrajectoryDecisionArtifact = (input) => {
    ...
    else if (input.failureLineage?.policy_snapshot?.recommended_action === "recontract" ||
...
//# sourceMappingURL=trajectory-controller.js.map