...
const recontractReasonForReport = (hasStaticContractBlockers) => hasStaticContractBlockers ? "adapter_contract_drift" : "adapter_runtime_drift";
export const buildAdapterDriftReport = (input) => {
    const staticBlockers = input.contractReviewArtifact.static_blockers ?? [];
    const missingTargetManifestKeys = input.failureLineage?.missing_target_manifest_keys ?? [];
    const signals = unique([
        ...(staticBlockers.length > 0 ? ["static_contract_blockers"] : []),
...
//# sourceMappingURL=adapter-drift.js.map