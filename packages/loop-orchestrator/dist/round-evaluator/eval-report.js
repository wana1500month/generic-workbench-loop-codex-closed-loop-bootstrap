...
import { adapterContractDocPath, adapterExamplePath, adapterHonestyCheck, adapterCriteriaGroundingCheck, adapterMeaningfulEvidenceCheck, adapterRuntimePath, artifactOnlyChecks, checkResult, evaluateVerificationProfile, expectedTargetSurfacesFor, fileSurfaceReservedCheck, fileWrittenCheck, independentTargetProbeCheck, isFailingCheck, isKnownCheck, isPassingCheck, isSatisfiedCheck, isVisualEvidencePath, liveVerificationPresentCheck, nonCarryForwardDerivedChecks, nonScoringDerivedChecks, pathExists, proofBoundaryIndependenceCheck, proofEvaluatorChecks, proofProvenanceAttestationCheck, proofScoreWeightsFor, releaseScoreWeightsFor, scoreFromResults, strictPartialCreditScore, successfulGradeRoundExecutionFor, unique } from "./shared.js";
const staticCheckLookup = (input) => {
    const hasMeaningfulCheck = input.contractArtifact.acceptance_checks.some((checkId) => !artifactOnlyChecks.has(checkId));
    const allChecksKnown = input.contractArtifact.acceptance_checks.every((checkId) => isKnownCheck(checkId));
    const carriedChecksAccepted = input.contractArtifact.carry_over_check_ids.every((checkId) => input.contractArtifact.acceptance_checks.includes(checkId));
    const roundContractHasReleaseScope = input.contractArtifact.release_gate_check_ids.length > 0 &&
        input.contractArtifact.proof_plan.length > 0 &&
        input.contractArtifact.pivot_triggers.length > 0;
    return {
        planner_brief_written: fileWrittenCheck("planner_brief_written", input.plannerBriefPath, "Planner brief"),
        plan_written: fileWrittenCheck("plan_written", input.planPath, "Run-local plan"),
        round_contract_written: fileWrittenCheck("round_contract_written", input.artifacts.contract_json_path, "Round contract artifact"),
        round_contract_is_testable: checkResult("round_contract_is_testable", hasMeaningfulCheck && allChecksKnown && carriedChecksAccepted ? "pass" : "fail", hasMeaningfulCheck && allChecksKnown && carriedChecksAccepted
        ...
        contract_review_written: fileWrittenCheck("contract_review_written", input.artifacts.contract_review_json_path, "Contract review artifact"),
        contract_review_quality: checkResult("contract_review_quality", input.contractReviewArtifact.decision === "revise"
            ? input.contractReviewArtifact.required_changes.length > 0
                ? "pass"
                : "fail"
            : input.contractReviewArtifact.required_changes.length === 0
                ? "pass"
                : "fail", input.contractReviewArtifact.decision === "revise"
        ...
        contract_agreement_written: fileWrittenCheck("contract_agreement_written", input.artifacts.contract_agreement_json_path, "Contract agreement artifact"),
        round_contract_scopes_release_qa: checkResult("round_contract_scopes_release_qa", roundContractHasReleaseScope ? "pass" : "fail", roundContractHasReleaseScope
        ...
        agreement_matches_review: checkResult("agreement_matches_review", (input.contractReviewArtifact.decision === "accept" &&
            ...
            ? "pass"
            : "fail", "The agreement status follows the review decision."),
        generator_plan_written: fileWrittenCheck("generator_plan_written", input.artifacts.generator_plan_json_path, "Generator plan artifact"),
        planner_context_surface_reserved: fileSurfaceReservedCheck("planner_context_surface_reserved", input.artifacts.planner_context_path, "Planner context handoff"),
        generator_brief_surface_reserved: fileSurfaceReservedCheck("generator_brief_surface_reserved", input.artifacts.generator_brief_path, "Generator brief handoff"),
        qa_review_surface_reserved: fileSurfaceReservedCheck("qa_review_surface_reserved", input.artifacts.qa_review_path, "QA review handoff"),
        handoff_is_resumable: checkResult("handoff_is_resumable", pathExists(input.artifacts.planner_context_path) &&
            ...
            ? "pass"
            : "fail", "The round keeps the full handoff surface and does not drop carried patch context."),
        evaluator_verdict_surface_reserved: fileSurfaceReservedCheck("evaluator_verdict_surface_reserved", input.artifacts.evaluator_verdict_json_path, "Evaluator verdict artifact"),
        patch_request_surface_reserved: fileSurfaceReservedCheck("patch_request_surface_reserved", input.artifacts.patch_request_json_path, "Patch request artifact"),
        previous_patch_request_addressed: checkResult("previous_patch_request_addressed", input.previousPatchTargetCheckIds.length === 0 || input.previousPatchRequestAddressed
            ? "pass"
            : "fail", input.previousPatchTargetCheckIds.length === 0
        ...
        eval_report_surface_reserved: fileSurfaceReservedCheck("eval_report_surface_reserved", input.artifacts.eval_report_path, "Eval report"),
        controller_decision_surface_reserved: fileSurfaceReservedCheck("controller_decision_surface_reserved", input.artifacts.controller_decision_path, "Controller decision handoff"),
        adapter_boundary_documented: checkResult("adapter_boundary_documented", existsSync(adapterContractDocPath) ? "pass" : "fail", existsSync(adapterContractDocPath)
            ? "Adapter contract document exists."
            : "Adapter contract document is missing."),
        adapter_runtime_present: checkResult("adapter_runtime_present", existsSync(adapterRuntimePath) ? "pass" : "fail", existsSync(adapterRuntimePath)
            ? "Adapter runtime source exists."
            : "Adapter runtime source is missing."),
        adapter_example_written: checkResult("adapter_example_written", existsSync(adapterExamplePath) ? "pass" : "fail", existsSync(adapterExamplePath)
            ? "Adapter example config exists."
            : "Adapter example config is missing.")
    };
};
const scoreDimensionApplicability = (input) => {
    if (input.dimension.skip_in_negotiation_modes?.includes(input.contractArtifact.negotiation_mode)) {
        return false;
    }
    if (input.dimension.requires_adapter && !input.loadedAdapter) {
        return false;
    }
    const expectedTargetSurfaces = input.loadedAdapter?.verification_profile?.profile.expected_target_surfaces ?? [];
    if (input.dimension.requires_target_surfaces?.length &&
        !input.dimension.requires_target_surfaces.some((surface) => expectedTargetSurfaces.includes(surface))) {
        return false;
    }
    if (input.dimension.required_core_probe_modes?.length) {
        const profileProbeModes = new Set(input.loadedAdapter?.verification_profile?.profile.core_probes?.map((probe) => probe.mode) ?? []);
        if (!input.dimension.required_core_probe_modes.some((mode) => profileProbeModes.has(mode))) {
            return false;
        }
    }
    return true;
...
const buildDimensionScores = (input) => {
    ...
    return dimensions.map((dimension) => {
        ...
        const passedItems = contributingChecks.filter((result) => result.status === "pass").length +
...
const targetSignalBlockingFailures = (input) => {
    const dimensionLookup = new Map((input.rubric.score_dimensions ?? []).map((dimension) => [
    ...
    return input.dimensionScores.filter((dimension) => {
        ...
        return (dimensionLookup.get(dimension.dimension_id)?.blocks_target_signal ?? true);
    });
};
export const buildEvalReport = (input) => {
    const evaluationCheckIds = input.contractAgreementArtifact.acceptance_checks.length > 0
        ? input.contractAgreementArtifact.acceptance_checks
        : input.contractArtifact.acceptance_checks;
    const thresholdAcceptanceCheckIds = evaluationCheckIds.filter((checkId) => checkId !== "target_signal_thresholds_met");
    const adapterResults = input.adapterExecutions.map((execution) => checkResult(`adapter_${execution.capability}`, execution.result.ok ? "pass" : "fail", execution.result.summary));
    ...
    const actionablePreviousPatchTargetCheckIds = input.previousPatchTargetCheckIds.filter((checkId) => !nonCarryForwardDerivedChecks.has(checkId));
    ...
    lookup.adapter_execution_healthy = checkResult("adapter_execution_healthy", input.loadedAdapter
        ? failedAdapterResults.length === 0
            ? "pass"
            : "fail"
        : "not_applicable", input.loadedAdapter
        ? failedAdapterResults.length === 0
    ...
    const adapterCheckPassed = (checkId) => adapterResults.some((result) => result.check_id === checkId && result.status === "pass");
    const releaseGateProbes = input.loadedAdapter?.verification_profile?.profile.core_probes?.filter((probe) => (probe.role ?? "supporting") === "release_gate" && probe.required !== false) ?? [];
    const browserReleaseGateProbeIds = new Set(releaseGateProbes
        .filter((probe) => probe.mode === "browser_journey")
        .map((probe) => probe.probe_id));
    const passedBrowserReleaseGateProbes = input.coreProbeResults.filter((probe) => probe.mode === "browser_journey" &&
        (probe.role ?? "supporting") === "release_gate" &&
        probe.ok &&
        probe.evidence_paths.length > 0);
    const targetManifestPublishesRuntime = Boolean(input.targetManifest?.app_url ||
        input.targetManifest?.api_base_url ||
        input.targetManifest?.health_url);
    lookup.build_brief_matches_user_intake = checkResult("build_brief_matches_user_intake", /runtime\/build-brief\.json/i.test(input.contractArtifact.objective) ||
        input.contractArtifact.non_goals.some((note) => /runtime\/build-brief\.json/i.test(note))
        ? "pass"
        : "fail", "The product round contract must keep runtime/build-brief.json as the source of truth.");
    lookup.target_root_created_or_updated = checkResult("target_root_created_or_updated", adapterCheckPassed("adapter_prepare_target") ? "pass" : "fail", adapterCheckPassed("adapter_prepare_target")
        ? "The adapter prepared the captured target root."
        : "The captured target root was not successfully prepared in this round.");
    lookup.core_workflows_have_user_visible_paths = checkResult("core_workflows_have_user_visible_paths", releaseGateProbes.length > 0 &&
        releaseGateProbes.every((probe) => input.coreProbeResults.some((result) => result.probe_id === probe.probe_id &&
            result.ok &&
            result.evidence_paths.length > 0))
        ? "pass"
        : "fail", releaseGateProbes.length > 0
    ...
    lookup.local_runtime_starts = checkResult("local_runtime_starts", adapterCheckPassed("adapter_run_target") || targetManifestPublishesRuntime ? "pass" : "fail", adapterCheckPassed("adapter_run_target") || targetManifestPublishesRuntime
    ...
    lookup.browser_journey_evidence_present = checkResult("browser_journey_evidence_present", browserReleaseGateProbeIds.size > 0 &&
        passedBrowserReleaseGateProbes.some((probe) => browserReleaseGateProbeIds.has(probe.probe_id))
        ? "pass"
        : "fail", browserReleaseGateProbeIds.size > 0
    ...
    lookup.no_scope_drift_from_build_brief = checkResult("no_scope_drift_from_build_brief", input.contractArtifact.non_goals.some((note) => /build-brief\.json/i.test(note)) ||
        ...
        ? "pass"
        : "fail", "The round contract should keep product scope anchored to runtime/build-brief.json.");
    ...
    const requiredSubjectiveMetricResults = subjectiveMetricResults.filter((metric) => metric.required !== false);
    const failedRequiredSubjectiveMetrics = requiredSubjectiveMetricResults.filter((metric) => metric.status === "fail");
    ...
    const prototypeBaselineSourceRound = typeof gradeRoundExecution?.result.metadata?.prototype_baseline_source_round === "number"
        ? gradeRoundExecution.result.metadata.prototype_baseline_source_round
        : undefined;
    const subjectiveJudgeDisabled = gradeRoundExecution?.result.metadata?.subjective_judge_disabled === true;
    ...
    const subjectiveJudgeNeedsEvaluatorDetail = subjectiveJudgeUnavailable
        ? `Status: needs_evaluator. Subjective-quality judge could not complete browser scoring${subjectiveJudgeTransportMode
        ...
        : undefined;
    const prototypeDeltaMetric = subjectiveMetricResults.find((metric) => metric.metric_id === "prototype_delta");
    const prototypeDeltaRequired = browserSurfaceExpected && input.round >= 2;
    const prototypeDeltaPassed = !prototypeDeltaRequired
        ? true
        : prototypeDeltaMetric?.status === "pass";
    lookup.subjective_quality_present = checkResult("subjective_quality_present", !browserSurfaceExpected
        ? "not_applicable"
        : subjectiveMetricResults.length > 0
            ? "pass"
            : "fail", !browserSurfaceExpected
    ...
    lookup.subjective_thresholds_met = checkResult("subjective_thresholds_met", !browserSurfaceExpected
        ? "not_applicable"
        : subjectiveMetricResults.length === 0
            ? "fail"
    ...
    lookup.visual_evidence_present = checkResult("visual_evidence_present", !browserSurfaceExpected
        ? "not_applicable"
    ...
    lookup.prototype_baseline_present = checkResult("prototype_baseline_present", !browserSurfaceExpected
        ? "not_applicable"
        : input.round < 2
            ? "pass"
        ...
        : input.round < 2
            ...
            : prototypeBaselinePresent
                ? `A persisted baseline screenshot is available for prototype-to-release comparison${prototypeBaselineSourcePhase
    ...
    lookup.prototype_baseline_valid = checkResult("prototype_baseline_valid", !browserSurfaceExpected
        ? "not_applicable"
        : input.round < 2
            ? "pass"
        ...
        : input.round < 2
            ...
            : prototypeBaselineValid
                ? `A valid initial prototype baseline is available${prototypeBaselineSourcePhase ? ` from '${prototypeBaselineSourcePhase}'` : ""}${typeof prototypeBaselineSourceRound === "number"
                    ? ` (source round ${prototypeBaselineSourceRound}).`
                    : "."}${prototypeBaselineSourceSemanticsDetail
                    ? ` ${prototypeBaselineSourceSemanticsDetail}`
                    : ""}`
                : prototypeBaselinePresent
                    ? `A baseline file exists${prototypeBaselineSourcePhase ? ` from '${prototypeBaselineSourcePhase}'` : ""}, but that source phase does not count as a valid initial prototype baseline.${prototypeBaselineSourceSemanticsDetail
                        ? ` ${prototypeBaselineSourceSemanticsDetail}`
                        : ""}`
                    : "Browser rounds after the initial prototype must keep a valid initial baseline before prototype_delta can pass.");
    lookup.prototype_delta_present = checkResult("prototype_delta_present", !browserSurfaceExpected
        ? "not_applicable"
    ...
    const preReleaseAcceptanceResults = unique([
        ...
        .map((checkId) => lookup[checkId] ??
        checkResult(checkId, "fail", `No evaluator rule is defined for check '${checkId}'.`));
    const failedPreReleaseAcceptanceResults = preReleaseAcceptanceResults.filter(isFailingCheck);
    ...
    lookup.release_blockers_recorded = checkResult("release_blockers_recorded", failedPreReleaseAcceptanceResults.length > 0 ||
        input.contractAgreementArtifact.status === "blocked" ||
        failedAdapterResults.length > 0
        ? releaseBlockerDetails.length > 0
            ? "pass"
            : "fail"
        : "pass", failedPreReleaseAcceptanceResults.length > 0 ||
        input.contractAgreementArtifact.status === "blocked" ||
        failedAdapterResults.length > 0
    ...
    const previousPatchResolved = actionablePreviousPatchTargetCheckIds.length === 0
        ? input.previousPatchTargetCheckIds.length === 0 || input.previousPatchRequestAddressed
        : actionablePreviousPatchTargetCheckIds.every((checkId) => {
            const targetResult = lookup[checkId] ??
                checkResult(checkId, "fail", `No evaluator rule is defined for carried check '${checkId}'.`);
            return isPassingCheck(targetResult);
        });
    lookup.previous_patch_request_resolved = checkResult("previous_patch_request_resolved", previousPatchResolved ? "pass" : "fail", actionablePreviousPatchTargetCheckIds.length === 0
    ...
    const acceptanceResultsWithoutThresholdCarry = thresholdAcceptanceCheckIds.map((checkId) => lookup[checkId] ??
        checkResult(checkId, "fail", `No evaluator rule is defined for check '${checkId}'.`));
    let check_results = unique([
        ...
        if (checkId.startsWith("adapter_")) {
            return (adapterResults.find((result) => result.check_id === checkId) ??
    ...
    const externalGrade = gradeRoundExecution?.result.score !== undefined ? gradeRoundExecution.result.score : undefined;
            ...
            ? (gradeRoundExecution?.verified_criteria_results.length
                ...
                :
                    input.adapterExecutions
                        ...
                        .flatMap((execution) => execution.verified_criteria_results)).map((criterion) => checkResult(criterion.criterion_id, criterion.status, criterion.summary))
            : [];
    ...
    const hasCriticalAdapterFailure = failedAdapterResults.some((result) => criticalAdapterFailures.has(result.check_id));
    let overall_verdict = input.contractAgreementArtifact.status === "blocked" || input.contractReviewArtifact.decision === "revise"
        ? "hold"
        : adapterVerdict === "hold" || hasCriticalAdapterFailure
            ...
            : failedPreReleaseAcceptanceResults.length > 0 ||
                ...
                ? "revise"
                : "advance";
    let contractCompleted = overall_verdict === "advance" &&
        acceptanceResultsWithoutThresholdCarry.every(isSatisfiedCheck);
    const controlPlaneResults = check_results.filter((result) => !proofEvaluatorChecks.has(result.check_id) &&
    ...
    const skepticalProofResults = input.loadedAdapter
        ? Array.from(proofEvaluatorChecks).map((checkId) => lookup[checkId] ??
    ...
    const hasProofExecution = input.adapterExecutions.some((execution) => execution.result.ok &&
    ...
    const raw_proof_score = input.loadedAdapter
        ? !hasProofExecution
            ? 0
        ...
        : 0;
    const proof_score = Number((input.loadedAdapter
    ...
    let release_score = Number((input.loadedAdapter
        ? control_plane_score * releaseScoreWeights.control_plane_score +
            proof_score * releaseScoreWeights.proof_score
        : control_plane_score * releaseScoreWeights.control_plane_score).toFixed(3));
    const releaseScoreCapDetails = [];
    if (browserSurfaceExpected && subjectiveMetricResults.length === 0) {
        release_score = Math.min(release_score, 0.59);
        releaseScoreCapDetails.push("Release score is capped at 0.590 because browser release quality did not report any subjective metrics.");
    }
    if (browserSurfaceExpected && !visualEvidencePresent) {
        release_score = Math.min(release_score, 0.59);
        releaseScoreCapDetails.push("Release score is capped at 0.590 because no rendered browser screenshots or traces were attached.");
    }
    if (browserSurfaceExpected && failedRequiredSubjectiveMetrics.length > 0) {
        release_score = Math.min(release_score, 0.79);
        releaseScoreCapDetails.push(`Release score is capped at 0.790 because required subjective metrics still fail: ${failedRequiredSubjectiveMetrics.map((metric) => metric.metric_id).join(", ")}.`);
    }
    if (browserSurfaceExpected && prototypeDeltaRequired && !prototypeBaselineValid) {
        release_score = Math.min(release_score, 0.84);
        releaseScoreCapDetails.push("Release score is capped at 0.840 because no valid initial prototype baseline was available for prototype_delta judging.");
    }
    else if (browserSurfaceExpected && prototypeDeltaRequired && !prototypeDeltaPassed) {
        release_score = Math.min(release_score, 0.84);
        releaseScoreCapDetails.push("Release score is capped at 0.840 because the current browser surface does not yet materially improve beyond the stored baseline.");
    }
    release_score = Number(release_score.toFixed(3));
    const coreOwnedEvaluatorProfileAttached = !input.loadedAdapter || input.loadedAdapter.verification_profile_source === "core";
    const threshold_results = {
        contract_completed: contractCompleted,
        minimum_control_plane_score_met: control_plane_score + 0.0005 >= input.rubric.minimum_control_plane_score,
        minimum_proof_score_met: proof_score + 0.0005 >= input.rubric.minimum_proof_score,
        minimum_release_score_met: release_score + 0.0005 >= input.rubric.target_total_score,
        adapter_required_met: input.rubric.target_signal_requires_adapter ? Boolean(input.loadedAdapter) : true,
        grade_score_required_met: input.rubric.target_signal_requires_grade_score
            ? externalGrade !== undefined &&
                (!browserSurfaceExpected || subjectiveMetricResults.length > 0)
            : true,
        core_probe_required_met: !input.loadedAdapter
            ? true
    ...
    const recomputeDimensionThresholds = () => {
        ...
        const dimensionGapDetails = failedDimensionScores.map((dimension) => `Dimension '${dimension.label}' scored ${dimension.score.toFixed(3)} below the minimum ${dimension.minimum_score.toFixed(3)}. ${dimension.detail}`);
    ...
    let { dimension_scores, failedDimensionScores, thresholdGapDetails } = recomputeDimensionThresholds();
    lookup.target_signal_thresholds_met = checkResult("target_signal_thresholds_met", !input.loadedAdapter
        ? "not_applicable"
        : contractCompleted && thresholdGapDetails.length === 0
            ? "pass"
            : "fail", !input.loadedAdapter
    ...
    check_results = check_results.map((result) => result.check_id === "target_signal_thresholds_met"
        ? lookup.target_signal_thresholds_met
        : result);
    const recomputedPreviousPatchResolved = actionablePreviousPatchTargetCheckIds.length === 0
        ? input.previousPatchTargetCheckIds.length === 0 || input.previousPatchRequestAddressed
        : actionablePreviousPatchTargetCheckIds.every((checkId) => {
            const targetResult = lookup[checkId] ??
                checkResult(checkId, "fail", `No evaluator rule is defined for carried check '${checkId}'.`);
    ...
    lookup.previous_patch_request_resolved = checkResult("previous_patch_request_resolved", recomputedPreviousPatchResolved ? "pass" : "fail", actionablePreviousPatchTargetCheckIds.length === 0
        ? "No previous patch request required resolution."
        : recomputedPreviousPatchResolved
            ? `Every carried check now passes: ${actionablePreviousPatchTargetCheckIds.join(", ")}.`
            : `At least one carried check is still unresolved: ${actionablePreviousPatchTargetCheckIds.join(", ")}.`);
    check_results = check_results.map((result) => result.check_id === "previous_patch_request_resolved"
    ...
    const acceptanceResults = evaluationCheckIds.map((checkId) => lookup[checkId] ??
        checkResult(checkId, "fail", `No evaluator rule is defined for check '${checkId}'.`));
    ...
    if (failedAcceptanceResults.length > 0 || failedAdapterResults.length > 0 || thresholdGapDetails.length > 0) {
        lookup.release_blockers_recorded = checkResult("release_blockers_recorded", "pass", thresholdGapDetails.length > 0
            ? "Release blockers were captured from failed checks, adapter failures, or unmet target thresholds."
            : "Release blockers were captured from failed checks, adapter failures, or blocked negotiation.");
        check_results = check_results.map((result) => result.check_id === "release_blockers_recorded"
            ? lookup.release_blockers_recorded
            : result);
    }
    if (contractCompleted && thresholdGapDetails.length > 0) {
        lookup.release_blockers_recorded = checkResult("release_blockers_recorded", "pass", "Release blockers were captured from failed checks, adapter failures, or unmet terminal thresholds.");
        check_results = check_results.map((result) => result.check_id === "release_blockers_recorded"
            ? lookup.release_blockers_recorded
            : result);
    }
    ...
    return {
        ...
        next_actions: input.contractReviewArtifact.decision === "revise"
