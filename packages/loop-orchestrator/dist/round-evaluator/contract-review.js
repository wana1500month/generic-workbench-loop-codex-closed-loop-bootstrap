import { artifactOnlyChecks, isKnownCheck, unique, verificationBoundaryIssues } from "./shared.js";
export const buildContractReviewArtifact = (input) => {
    ...
    const duplicateChecks = input.contractArtifact.acceptance_checks.filter((checkId, index, allChecks) => allChecks.indexOf(checkId) !== index);
    const carryOverChecksNotAccepted = input.contractArtifact.carry_over_check_ids.filter((checkId) => !input.contractArtifact.acceptance_checks.includes(checkId));
    ...
    if (input.contractArtifact.carry_over_patch_ids.length > 0 &&
    ...
    if (carryOverChecksNotAccepted.length > 0) {
        concerns.push(`The draft contract does not promise to close carried checks: ${carryOverChecksNotAccepted.join(", ")}.`);
    ...
    if (input.loadedAdapter) {
        ...
        if (boundaryIssues.length > 0) {
            ...
            concerns.push(`Independent proof boundary is incomplete: ${boundaryIssues.join(" ")}`);
            requiredChanges.push("Fix the adapter contract before retrying: attach a distinct verification_provider, a core-owned evaluator profile, and required browser_journey/http_json release-gate probes for independent target verification.");
        }
        if (!input.loadedAdapter.verification_profile) {
            staticBlockers.push("No core-owned evaluator profile is attached, so target-specific criteria would stay adapter-authored.");
            concerns.push("No core-owned evaluator profile is attached, so target-specific criteria would stay adapter-authored.");
            requiredChanges.push("Attach a core-owned evaluator bundle via --target-family or rubric.evaluator_profile_path, or use --evaluator-profile for an explicit override.");
        }
        else if (input.loadedAdapter.verification_profile_source !== "core") {
            requiredChanges.push("Move verification profile ownership into the harness: select it through --target-family or rubric.evaluator_profile_path, and reserve --evaluator-profile for explicit overrides instead of adapter.json.");
        }
    }
    else {
    ...
    return {
        ...
        approved_checks: requiredChanges.length > 0
