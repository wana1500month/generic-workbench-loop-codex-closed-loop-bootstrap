...
const slugifyIdentifier = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
const featureIdFor = (prefix, index, label) => `${prefix}-${slugifyIdentifier(label) || String(index + 1).padStart(2, "0")}`;
export const createDurableMemoryPaths = (rootDirectory) => ({
...
export const buildFeatureLedger = (input) => ({
...
export const buildProgressJsonl = (input) => `${JSON.stringify({
    timestamp: "bootstrap",
...
export const buildDoneWhenMarkdown = (input) => [
    "# Done When",
    "",
...
const mergeDurableMemoryContext = (idea, intake) => ({
    ...
    coreFeatures: uniqueList((intake?.core_features?.length ?? 0) > 0
        ? intake?.core_features ?? []
        : idea.user_goals.length > 0
            ? idea.user_goals
            : [idea.summary]),
    qualityBar: uniqueList((intake?.quality_bar?.length ?? 0) > 0 ? intake?.quality_bar ?? [] : idea.quality_bar),
    constraints: uniqueList((intake?.constraints?.length ?? 0) > 0 ? intake?.constraints ?? [] : idea.constraints),
    mustNotBreak: uniqueList(intake?.must_not_break ?? []),
    targetScore: intake?.target_score,
    maxRounds: intake?.max_rounds
});
export const loadDurableMemoryContext = async (idea) => {
    const rootDirectory = dirname(idea.source_path);
    const intake = await loadJsonIfExists(join(rootDirectory, "intake.json"));
    return {
        rootDirectory,
        context: mergeDurableMemoryContext(idea, intake)
    };
};
export const scaffoldDurableMemoryArtifacts = async (rootDirectory, context) => {
...
export const ensureDurableMemoryArtifacts = async (rootDirectory, context) => {
    const paths = createDurableMemoryPaths(rootDirectory);
...
export const detectDurableMemoryPaths = async (rootDirectory) => {
    const paths = createDurableMemoryPaths(rootDirectory);
    return {
...
//# sourceMappingURL=durable-memory.js.map