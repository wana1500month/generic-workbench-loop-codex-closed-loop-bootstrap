...
import { buildExecutorModePrompt, experimentalExecutorRuntimeWarning } from "./codex-agent-manifest.js";
...
const roundEnhancementSkillPath = join(repoRoot, ".agents", "skills", "round-enhancement", "SKILL.md");
...
const agentFailureWarning = (agentName, details) => `Codex ${agentName} agent fallback: ${details}`;
const disabledStageWarning = (stage) => `${stage} agent execution disabled by ${stageDisableEnv[stage]}=1.`;
...
const failureDetails = (execution) => execution.error ?? trimString(execution.stderr) ?? `exit code ${execution.code}`;
const maybeDisabledStage = (stage, fallbackValue) => process.env[stageDisableEnv[stage]] === "1"
    ? {
        value: fallbackValue,
        runtimeWarnings: [agentFailureWarning(stage, disabledStageWarning(stage))]
    }
    : undefined;
const skillPrompt = (skillName, prompt) => [`Use $${skillName}.`, "", prompt].join("\n");
const appServerFailureWarning = (agentName, details) => `App Server ${agentName} enhancement fallback: ${details}`;
    ...
    if (execution.code !== 0 || execution.error || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...
                agentFailureWarning("planner", failureDetails(execution))
            ]
        };
    }
    const parsed = parseJsonResponse(execution.responseText);
    if (!parsed) {
        return {
            value: { scenario: input.scenario, plan: input.plan },
            runtimeWarnings: [
...
export const enhanceContractReviewWithCodex = async (input) => {
    ...
    if (execution.code !== 0 || execution.error || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("contract-review", failureDetails(execution))
            ]
        };
    }
    const parsed = parseJsonResponse(execution.responseText);
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("contract-review", "response was not valid JSON; using deterministic contract review.")
            ]
        };
    }
...
export const enhanceGeneratorPlanWithCodex = async (input) => {
    ...
    if (execution.code !== 0 || execution.error || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("generator-plan", failureDetails(execution))
            ]
        };
    }
    const parsed = parseJsonResponse(execution.responseText);
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("generator-plan", "response was not valid JSON; using deterministic generator plan.")
            ]
        };
    }
    return {
...
export const enhanceEvalReportWithCodex = async (input) => {
    ...
    if (execution.code !== 0 || execution.error || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("evaluator", failureDetails(execution))
            ]
        };
    }
    const parsed = parseJsonResponse(execution.responseText);
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                agentFailureWarning("evaluator", "response was not valid JSON; using deterministic evaluator report.")
            ]
        };
    }
...
export const enhancePlanWithAppServer = async (input) => {
    ...
    try {
        ...
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("planner", error instanceof Error
    ...
    if (execution.status !== "completed" || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("planner", execution.status === "completed"
    ...
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("planner", "response was not valid JSON; using deterministic planner output.")
            ]
        };
    }
    const focusAreas = Array.isArray(parsed.planner_focus_areas)
...
export const enhanceContractReviewWithAppServer = async (input) => {
    ...
    try {
        ...
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("contract-review", error instanceof Error
    ...
    if (execution.status !== "completed" || !execution.reviewText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("contract-review", execution.status === "completed"
                    ? "empty review output; using deterministic contract review."
                    : `review turn ended with status '${execution.status}'.`)
            ]
        };
    }
    const parsed = parseJsonResponse(execution.reviewText);
    if (!parsed) {
        return {
            value: input.contractReviewArtifact,
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("contract-review", "review output was not valid JSON; using deterministic contract review.")
            ]
        };
    }
    const approvedChecks = stringList(parsed.approved_checks).filter((checkId) => input.contractArtifact.acceptance_checks.includes(checkId));
...
export const enhanceGeneratorPlanWithAppServer = async (input) => {
    ...
    try {
        ...
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("generator-plan", error instanceof Error
    ...
    if (execution.status !== "completed" || !execution.responseText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("generator-plan", execution.status === "completed"
    ...
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("generator-plan", "response was not valid JSON; using deterministic generator plan.")
            ]
        };
    }
    return {
...
export const enhanceEvalReportWithAppServer = async (input) => {
    ...
    try {
        ...
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("evaluator", error instanceof Error
    ...
    if (execution.status !== "completed" || !execution.reviewText) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("evaluator", execution.status === "completed"
    ...
    if (!parsed) {
        return {
            ...
            runtimeWarnings: [
                ...(preparedPrompt.warning ? [preparedPrompt.warning] : []),
                appServerFailureWarning("evaluator", "review output was not valid JSON; using deterministic evaluator report.")
            ]
        };
    }
    const mergedVerdict = strictestVerdict(input.evalReport.overall_verdict, parsed.overall_verdict);
