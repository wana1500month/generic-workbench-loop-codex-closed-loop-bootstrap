...
const normalizeHostname = (hostname) => hostname.trim().replace(/^\[/, "").replace(/\]$/, "").toLowerCase();
const envFlagEnabled = (name) => truthyEnvValues.has((process.env[name] ?? "").trim().toLowerCase());
const allowNonlocalTargetUrls = () => envFlagEnabled("HARNESS_ALLOW_NONLOCAL_TARGET_URLS") ||
...
const isBlockedPrivateOrMetadataHost = (hostname) => {
    const normalized = normalizeHostname(hostname);
...
export const validateTargetUrlPolicy = (value) => {
    let parsed;
    try {
    ...
    if (!allowNonlocalTargetUrls()) {
        return {
            ok: false,
            reason: "Only localhost and loopback target URLs are allowed by default. Set HARNESS_ALLOW_NONLOCAL_TARGET_URLS=1 to opt into nonlocal targets."
        };
    }
...
export const assertAllowedTargetUrl = (value, context) => {
...
//# sourceMappingURL=target-url-policy.js.map