...
import { buildBootstrapAnswersFromSeed, createBootstrapArtifactPaths, normalizeBootstrapTargetFamily, scaffoldBootstrapArtifacts } from "./bootstrap.js";
import { ensureDurableMemoryArtifacts, loadDurableMemoryContext } from "./durable-memory.js";
import { appendJsonLine, loadJson, loadJsonIfExists, nextRunId, pathExists, repoRoot, removeIfExists, resolveRunsDirectory, writeJson, writeText } from "./file-system.js";
...
import { buildEvaluationPolicy, evaluationPolicyMarkdownPathForRun, evaluationPolicyPathForRun, writeEvaluationPolicyArtifacts } from "./evaluation-policy.js";
import { buildReadinessReport, writeReadinessReportArtifacts } from "./readiness-doctor.js";
import { runtimeStatePathsForRun } from "./runtime-state.js";
import { buildOperatorSurfaceSessionProjection, loadPreparedSessionSeed, writeSessionPreparationArtifacts } from "./session-artifacts.js";
...
const threadBindingStates = new Set([
    "bound",
    "assumed",
    "unbound"
]);
const workspaceModes = new Set(["local", "worktree"]);
const unique = (values) => [...new Set(values.map((value) => value.trim()).filter(Boolean))];
const readThreadBindingStateFromEnv = () => {
    const value = process.env.HARNESS_THREAD_BINDING_STATE?.trim();
    return value && threadBindingStates.has(value)
        ? value
        : undefined;
};
const inferWorkspaceMode = (projectMode) => (projectMode === "new" ? "worktree" : "local");
const normalizeRelativePath = (path) => path.replace(/\\/g, "/");
const resolveTargetRootForIntake = (targetRoot) => resolve(repoRoot, targetRoot);
const eventsPathForFrontDoorSession = (sessionPath) => sessionPath.replace(/\.json$/u, ".events.jsonl");
const loadReadyFrontDoorSession = async (frontDoorSessionPath) => {
    const resolvedPath = resolve(repoRoot, frontDoorSessionPath);
    const artifact = await loadJson(resolvedPath);
    if (artifact.lane !== "product_build") {
        throw new Error(`Front-door session ${frontDoorSessionPath} is not a product_build session.`);
    }
    if (artifact.phase !== "ready_for_prepare") {
        throw new Error(`Front-door session ${frontDoorSessionPath} is ${artifact.phase}, not ready_for_prepare.`);
    }
    if (!artifact.intake.target_root?.trim()) {
        throw new Error(`Front-door session ${frontDoorSessionPath} is missing intake.target_root.`);
    }
    return {
        artifact,
        path: resolvedPath,
...
const isFallbackThreadId = (threadId) => threadId === fallbackFrontDoorThreadId;
...
const buildSessionIdeaFromIntake = (input) => {
    const title = input.intake?.product_title ??
    ...
    const userGoals = input.intake?.core_features && input.intake.core_features.length > 0
...
const encodeReadyToStartKey = (value) => encodeURIComponent(value).replace(/\./g, "%2E");
export const legacyReadyToStartMarkerPathForRuns = (runsDirectory) => join(runsDirectory, "ready-to-start-session.json");
export const readyToStartIndexDirectoryForRuns = (runsDirectory) => join(runsDirectory, "ready-to-start");
export const readyToStartMarkerPathForRuns = (runsDirectory) => join(readyToStartIndexDirectoryForRuns(runsDirectory), "latest.json");
export const readyToStartMarkerPathForRun = (runsDirectory, runId) => join(readyToStartIndexDirectoryForRuns(runsDirectory), "by-run", `${encodeReadyToStartKey(runId)}.json`);
export const readyToStartMarkerPathForThread = (runsDirectory, threadId) => join(readyToStartIndexDirectoryForRuns(runsDirectory), "by-thread", `${encodeReadyToStartKey(threadId)}.json`);
export const loadReadyToStartSessionMarker = async (runsDirectory) => (await loadJsonIfExists(readyToStartMarkerPathForRuns(runsDirectory))) ??
    (await loadJsonIfExists(legacyReadyToStartMarkerPathForRuns(runsDirectory)));
const loadReadyToStartSessionMarkerForRun = async (runsDirectory, runId) => loadJsonIfExists(readyToStartMarkerPathForRun(runsDirectory, runId));
const loadReadyToStartSessionMarkerForThread = async (runsDirectory, threadId) => loadJsonIfExists(readyToStartMarkerPathForThread(runsDirectory, threadId));
export const writeReadyToStartSessionMarker = async (runsDirectory, marker) => {
    const writes = [
    ...
    if (marker.thread_id && !isFallbackThreadId(marker.thread_id)) {
        writes.push(writeJson(readyToStartMarkerPathForThread(runsDirectory, marker.thread_id), marker));
    }
    await Promise.all(writes);
};
export const clearReadyToStartSessionMarker = async (runsDirectory, marker) => {
    const removals = [
    ...
    if (marker.thread_id && !isFallbackThreadId(marker.thread_id)) {
        removals.push(removeIfExists(readyToStartMarkerPathForThread(runsDirectory, marker.thread_id)));
    }
    await Promise.all(removals);
};
export const loadPreparedSessionSeedForRun = async (runDirectory) => {
...
const markerMatchesPreparedThread = (marker, currentThreadId) => {
...
const matchesPreparedThread = (sessionStatus, currentThreadId, allowAssumedForeground) => {
...
const isPreparedRunAwaitingStart = async (input) => {
    ...
    if (!matchesPreparedThread(input.sessionStatus, input.currentThreadId, input.allowAssumedForeground === true)) {
        return false;
    }
    return !(await pathExists(join(input.runDirectory, "summary.json")));
};
export const findLatestPreparedRunAwaitingStart = async (runsDirectory, currentThreadId, options = {}) => {
    try {
        ...
        if (markerCandidate &&
            ...
            const markerSessionStatus = await loadJsonIfExists(runtimeStatePathsForRun(markerCandidate.run_directory).sessionStatusPath);
            const markerPreparedSeed = await loadPreparedSessionSeedForRun(markerCandidate.run_directory);
            if (await isPreparedRunAwaitingStart({
                runDirectory: markerCandidate.run_directory,
                currentThreadId,
                allowAssumedForeground: options.allowAssumedForeground === true && Boolean(options.runId),
        ...
        if (options.runId) {
            ...
            const [sessionStatus, preparedSeed] = await Promise.all([
                loadJsonIfExists(runtimePaths.sessionStatusPath),
        ...
        if (candidates.length > 1) {
            throw new Error(`Multiple ready_to_start runs match this start request: ${candidates
...
export const prepareSessionRun = async (input) => {
    ...
    const resolvedRunDirectory = input.runDirectory
        ...
        : undefined;
    const runId = resolvedRunDirectory !== undefined
        ? basename(resolvedRunDirectory)
        : await nextRunId(runsDirectory);
    const runDirectory = resolvedRunDirectory ?? join(runsDirectory, runId);
    ...
    const discoveryThreadId = frontDoorSession?.artifact.thread_id?.trim() || undefined;
    if (envThreadId &&
        discoveryThreadId &&
        !isFallbackThreadId(discoveryThreadId) &&
        envThreadId !== discoveryThreadId) {
        throw new Error(`Front-door session belongs to thread ${discoveryThreadId}, but current CODEX_THREAD_ID is ${envThreadId}.`);
    }
    const effectiveThreadId = envThreadId ?? (isFallbackThreadId(discoveryThreadId) ? undefined : discoveryThreadId);
    const effectiveThreadBindingState = envThreadId
        ? "bound"
    ...
    await ensureDurableMemoryArtifacts(durableMemory.rootDirectory, durableMemory.context);
    let intake = await loadJsonIfExists(join(durableMemory.rootDirectory, "intake.json"));
    ...
    const targetScore = input.targetScore ?? intake?.target_score ?? evaluationPolicy.target_total_score;
    ...
    if (bootstrapTargetFamily) {
        if (!intake?.target_root?.trim()) {
            throw new Error("Prepared product session is missing intake.target_root. Refusing to scaffold a product adapter bundle without execution target identity.");
        }
        ...
        if (missingProductFields.length > 0) {
            throw new Error(`Prepared product session is missing required product intake fields: ${missingProductFields.join(", ")}`);
        }
        const bootstrapResult = await scaffoldBootstrapArtifacts(buildBootstrapAnswersFromSeed({
            ...
            verificationSurfaces: intake?.verification_surfaces ?? evaluationPolicy.evidence_surfaces,
            workflowChecks: intake?.workflow_checks?.map((check) => ({
    ...
    if (frontDoorSession) {
        const refreshedIntake = await loadJsonIfExists(join(durableMemory.rootDirectory, "intake.json"));
        ...
        if (refreshedIntake?.target_root?.trim()) {
            await writeJson(join(resolveTargetRootForIntake(refreshedIntake.target_root), "intake.json"), refreshedIntake);
        }
    }
    const rubric = await loadJson(resolvedRubricPath);
    ...
    if (bootstrapTargetFamily && !preparedValidationBundle) {
        ...
        preparedValidationBundle = {
            ...
            ...(rubric.evaluator_profile_path
                ? {
                    evaluator_profile_path: resolve(dirname(resolvedRubricPath), rubric.evaluator_profile_path)
                }
                : hasGeneratedProfile
                    ? {
                        evaluator_profile_path: resolve(bootstrapPaths.generatedVerificationProfilePath)
                    }
    ...
    const workspaceMode = input.workspaceMode && workspaceModes.has(input.workspaceMode)
        ? input.workspaceMode
    ...
    const result = await writeSessionPreparationArtifacts({
        ...
        ...(readinessReport.blockers.length > 0
            ? {
                externalBlockers: readinessReport.blockers.map((blocker) => `${blocker.code}: ${blocker.human_explanation} Fix: ${blocker.how_to_fix}`)
            }
            : {}),
    ...
    if (isProductBuild && preparedValidationBundle?.evaluator_profile_path) {
        const evaluatorProfile = await loadJson(preparedValidationBundle.evaluator_profile_path);
        ...
        if (integrityErrors.length > 0) {
            throw new Error(`Prepared product session failed evaluator profile integrity checks:\n${integrityErrors
                .map((error) => `- ${error}`)
                .join("\n")}`);
        }
    }
    ...
    await writeText(join(runDirectory, "runtime", "ready-to-start.md"), [
        "# Ready to start",
        "",
        `Run ID: ${runId}`,
        frontDoorSession?.artifact.session_id
    ...
    await writeReadyToStartSessionMarker(runsDirectory, {
        ...
        ...(frontDoorSession?.path
            ? {
                front_door_session_path: normalizeRelativePath(relative(repoRoot, frontDoorSession.path))
            }
            : {})
    });
    if (frontDoorSession) {
...
//# sourceMappingURL=prepare-session.js.map