...
export const finalizeNoopTerminalResume = async (input) => {
    ...
    const transportProtocolPath = await writeTransportProtocol({
        ...
        activePhase: input.resumePhase ?? input.restoredRun.interruptedRound?.resumeFromPhase,
    ...
    const noopRuntimeEvents = mergeRuntimeEvents([
        ...
        buildRuntimeEvent("resume.noop_terminal", `Run '${input.runId}' already ended with terminal stop reason '${input.restoredStopReason}'. Resume returned without opening a new round. Re-run with --force-reopen-terminal to override this default.`, {
    ...
    const summary = {
        ...
        ...(input.currentResumeIdentity.adapter_contract_sha256
            ? {
                adapter_contract_sha256: input.currentResumeIdentity.adapter_contract_sha256
            }
            : {}),
        ...(input.currentResumeIdentity.evaluator_bundle_sha256
            ? {
                evaluator_bundle_sha256: input.currentResumeIdentity.evaluator_bundle_sha256
            }
            : {}),
        ...
        ...(await detectDurableMemoryPaths(dirname(input.restoredRun.summary.idea_path ?? defaultIdeaPath))),
        ...
        verification_provider_id: input.loadedAdapter?.contract.verification_provider?.provider_id ??
...
//# sourceMappingURL=noop-terminal-resume.js.map