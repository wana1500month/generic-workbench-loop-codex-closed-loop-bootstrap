import { canonicalCodexCheckpointStopReason, legacyCurrentThreadHandoffStopReason } from "./stop-reason.js";
export const pausedStopReasons = new Set([
...
const ageFor = (value, now) => {
    const parsed = parseTimestamp(value);
    return parsed === undefined ? undefined : Math.max(now - parsed, 0);
};
const firstDefined = (...values) => values.find((value) => value !== undefined);
export const phaseBudgetToStallThresholdMs = (phaseTimeoutMs) => {
...
export const heartbeatTimestampForRuntime = (input) => firstDefined(input.liveState?.heartbeat_at, input.controllerLease?.heartbeat_at, input.roundPhase?.heartbeat_at);
export const progressTimestampForRuntime = (input) => firstDefined(input.liveState?.last_progress_at, input.roundPhase?.last_progress_at, input.controllerLease?.last_progress_at, input.roundPhase?.phase_started_at, input.liveState?.updated_at, input.roundPhase?.updated_at, input.controllerLease?.updated_at);
export const assessRuntimeHealth = (input) => {
    const now = input.now ?? Date.now();
    const phaseTimeoutMs = firstDefined(input.liveState?.phase_timeout_ms, input.roundPhase?.phase_timeout_ms);
    const stallThresholdMs = input.stallThresholdMs ??
        firstDefined(input.liveState?.stall_threshold_ms, input.roundPhase?.stall_threshold_ms) ??
    ...
    const heartbeatStale = heartbeatAgeMs !== undefined &&
        heartbeatAgeMs >
            (input.heartbeatStaleMs ?? defaultHeartbeatStaleMs);
    const progressStale = progressAgeMs !== undefined && progressAgeMs > stallThresholdMs;
    const transportStale = transportEventAgeMs !== undefined &&
        transportEventAgeMs >
    ...
    const paused = liveExecutionState === "paused" ||
    ...
    const controllerStatus = executionState === "completed"
        ...
        : executionState === "paused"
            ...
            : executionState === "stalled"
                ...
                : executionState === "failed"
                    ? "failed"
                    : "running";
    const summary = executionState === "stalled"
        ? `No recorded progress for ${Math.round((progressAgeMs ?? 0) / 1000)}s while the controller heartbeat stayed ${heartbeatStale ? "stale" : "fresh"}.`
        : executionState === "paused"
            ? phaseStatus === "awaiting_codex_work"
                ...
                : phaseStatus === "awaiting_human_input"
                    ...
                    : phaseStatus === "awaiting_external_condition"
                        ...
                        : `Run is paused${stopReason ? ` (${stopReason}).` : "."}`
            : executionState === "completed"
