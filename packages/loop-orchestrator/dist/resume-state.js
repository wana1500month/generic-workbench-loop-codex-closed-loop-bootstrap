...
import { buildActiveContractFrame, targetCheckIdsFromPatchRequest, unresolvedSignatureFor } from "./attempt-lifecycle.js";
import { buildFailureLineageArtifact, loadFailureLineageArtifact } from "./failure-lineage.js";
...
import { readControllerLeaseArtifact, readRuntimeLiveStateArtifact, readRuntimeRoundPhaseArtifact, readSupervisorStateArtifact, readTransportStateArtifact, runtimeStatePathsForRun } from "./runtime-state.js";
import { assessRuntimeHealth, defaultHeartbeatStaleMs } from "./runtime-health.js";
import { normalizeRunStopReason } from "./stop-reason.js";
const isImproved = (nextScore, currentBest) => currentBest === undefined || nextScore > currentBest + 0.001;
const scoreDeltasFromHistory = (history) => history.slice(1).map((summary, index) => Number((summary.total_score - history[index].total_score).toFixed(3)));
export const failureLineageForEvalReport = (input) => {
    const normalized = "generated_at" in input
        ? { evalReport: input, loadedAdapter: undefined, previousRoundSummary: undefined }
        : input;
    if (!normalized.evalReport) {
        return undefined;
    }
    const targetManifest = normalized.evalReport.adapter_results.find((execution) => execution.capability === "run_target" && execution.result.ok)?.result.target_manifest;
...
const plateauCountFromHistory = (history) => {
    let bestScore;
    let plateauCount = 0;
...
const repeatedUnresolvedCountFromHistory = (history) => {
    let lastSignature;
    let repeatedCount = 0;
    for (const summary of history) {
...
const activeContractFrameForHistory = async (history) => {
    for (const round of [...history].reverse()) {
        ...
        const [contractArtifact, contractAgreementArtifact] = await Promise.all([
            loadJson(round.contract_path),
            loadJson(round.contract_agreement_path)
        ]);
...
const resolvedRunDirectory = (runPath) => resolve(runPath);
const roundDirectoryPattern = /^round-(\d+)$/;
const loadRoundSummariesFromDisk = async (runDirectory) => {
    ...
    const summaries = await Promise.all(entries
        ...
        return loadJsonIfExists(roundSummaryPath);
    }));
    return summaries
        .filter((summary) => Boolean(summary))
        .sort((left, right) => left.round - right.round);
};
const mergeRoundHistory = (summaryHistory, diskHistory) => [...new Map([...summaryHistory, ...diskHistory]
        .sort((left, right) => left.round - right.round)
        .map((summary) => [summary.round, summary])).values()];
const bestRoundSummaryFromHistory = (history) => {
    let bestRoundSummary;
    for (const summary of history) {
...
const hydrateSummaryFromHistory = (input) => {
    ...
    const stopReason = normalizeRunStopReason(latestRoundSummary?.round_stop_reason &&
    ...
    return {
        ...
        control_plane_score: latestRoundSummary?.control_plane_score ?? input.summary?.control_plane_score ?? 0,
...
const interruptedRoundStateFor = (input) => {
    ...
    return {
        ...
        roundDirectory: join(input.runDirectory, `round-${String(input.runtimeRoundPhase.round).padStart(3, "0")}`),
        resumeFromPhase: input.runtimeRoundPhase.phase,
        phaseStatus: input.runtimeRoundPhase.status
    };
};
export const restoreRunState = async (runPath) => {
    ...
    const [summary, scenario, plan, rubric, runtimeLiveState, runtimeRoundPhase, controllerLease, transportState, supervisorState, diskHistory] = await Promise.all([
        loadJsonIfExists(summaryPath),
        loadJsonIfExists(plannedScenarioPath),
        loadJsonIfExists(planPath),
        loadJson(rubricPath),
    ...
    const initializationIncomplete = initializationMissingArtifacts.length > 0 && history.length === 0;
    if (initializationMissingArtifacts.length > 0 && !initializationIncomplete) {
        throw new Error(`Missing run-scoped planning artifacts for resume: ${initializationMissingArtifacts
            .map((path) => basename(path))
            .join(", ")}.`);
    }
    const hydratedScenario = scenario ??
        (initializationIncomplete
            ? buildScenarioFromIdea(await readIdeaBrief(summary?.idea_path ?? defaultIdeaPath))
            : undefined);
    if (!hydratedScenario) {
        throw new Error(`Could not resolve planning scenario state while restoring run '${runId}'.`);
    }
    ...
    const previousTrajectoryDecision = previousTrajectoryDecisionPath
        ? await loadJson(previousTrajectoryDecisionPath)
        : undefined;
    const latestEvalReport = latestRoundSummary?.eval_report_path
        ? await loadJson(latestRoundSummary.eval_report_path)
        : undefined;
    const latestFailureLineage = (await loadFailureLineageArtifact(latestRoundSummary?.failure_lineage_path)) ??
        failureLineageForEvalReport({
            evalReport: latestEvalReport,
            previousRoundSummary,
            loadedAdapter: undefined
        });
    const previousFailureLineage = previousRoundSummary
        ? (await loadFailureLineageArtifact(previousRoundSummary.failure_lineage_path)) ??
            failureLineageForEvalReport({
                evalReport: previousRoundSummary.eval_report_path
                    ? await loadJson(previousRoundSummary.eval_report_path)
                    : undefined,
                loadedAdapter: undefined
            })
        : undefined;
    ...
    return {
        ...
        latestRoundSummary,
        previousRoundSummary,
        bestScore: hydratedSummary.best_scoring_total_score ?? hydratedSummary.total_score,
        bestControlPlaneScore: hydratedSummary.best_scoring_control_plane_score ??
            hydratedSummary.control_plane_score,
        bestProofScore: hydratedSummary.best_scoring_proof_score ?? hydratedSummary.proof_score,
        bestReleaseScore: hydratedSummary.best_scoring_release_score ?? hydratedSummary.release_score,
        bestThresholdResults: hydratedSummary.best_scoring_threshold_results ??
            hydratedSummary.threshold_results,
        bestRound: hydratedSummary.best_round ??
            hydratedSummary.terminal_round ??
            Math.max(history.length, 1),
        bestEvalReportPath: history.find((round) => round.round === (hydratedSummary.best_round ?? hydratedSummary.terminal_round))
            ?.eval_report_path ?? latestRoundSummary?.eval_report_path ?? "",
        bestPatchRequestPath: history.find((round) => round.round === (hydratedSummary.best_round ?? hydratedSummary.terminal_round))
        ...
        summaryWasRecovered: !summary || diskHistory.length !== (summary?.round_history ?? []).length,
        repairNotes: [
...
export const buildRemediationHistory = (input) => {
    ...
    return {
        ...
        repeated_failure_classification_count: input.latestFailureLineage?.policy_snapshot
        ...
        contradiction_count: input.latestFailureLineage?.contradictory_witness_assertion_ids.length ?? 0,
        environment_blocked: input.latestFailureLineage?.failure_classification === "environment_blocked",
        score_deltas: input.scoreDeltas.slice(-3),
        patch_entropy: Number((patchTargetCheckIds.length > 0
        ...
        patch_authority_state: input.latestFailureLineage?.policy_snapshot?.patch_authority_state,
