...
import { appendJsonLine, loadJsonIfExists, loadJsonLinesIfExists, writeJson, writeText } from "./file-system.js";
...
const sessionReviewBoundaries = [
    "diff_ready",
    "milestone_scope_complete",
    "risk_gate",
    "release_candidate"
];
const sessionApprovalBoundaries = [
    "scope_change",
    "destructive_change",
    "external_access",
    "deploy",
...
const titleCase = (value) => value
    ...
    .join(" ");
const deriveTitleFromSummary = (summary) => {
    const cleaned = summary
        ?.replace(/[.!?。！？]+$/u, "")
        .replace(/^(?:build|create|make|prototype|ship)\s+(?:me\s+|us\s+|a\s+|an\s+|the\s+)?/i, "")
...
const inferExecutionPreference = (deliveryLevel) => {
...
const inferPrimarySurface = (targetFamily) => {
...
const inferSecondarySurfaces = (input) => {
    ...
    if (input.targetFamily === "fullstack-app" ||
    ...
    const result = [...surfaces].filter((surface) => surface !== inferPrimarySurface(input.targetFamily));
    return result.length > 0 ? result : undefined;
};
const inferAuthMode = (intake) => {
...
const bulletList = (values, emptyText) => values.length > 0 ? values.map((value) => `- ${value}`).join("\n") : `- ${emptyText}`;
const relativeToRun = (runDirectory, path) => relative(runDirectory, path).replace(/\\/g, "/");
const sessionReadinessForStatus = (status) => {
    switch (status) {
        case "asking":
        case "needs_steering":
            return "needs_input";
...
const sessionAttentionForStatus = (status) => {
    switch (status) {
        case "asking":
        case "needs_steering":
            return "human";
...
const buildSessionBindingArtifact = (input) => {
    const surface = input.transportMode === "app-server"
        ? "app-server"
        : input.transportMode === "current-thread" &&
            input.threadBindingState !== "unbound"
            ? "current-thread"
            : "manual-protocol";
    const bindingState = surface === "app-server"
        ? input.threadId
            ? "bound"
            : "unbound"
...
const buildSessionActiveCheckpointArtifact = (input) => {
    ...
    const defaultSkill = input.transportMode === "current-thread" ? "loop-control" : "run-resume";
    return {
...
const buildTargetManifestHints = (intake) => ({
...
const buildOpenQuestions = (input) => {
    ...
    return Array.from(entries.reduce((map, entry) => map.set(entry.prompt, entry), new Map()).values());
};
export const buildSessionStatusArtifact = (input) => {
    ...
    return {
        ...
        artifacts: {
            ...
            session_status_events_path: relativeToRun(input.runDirectory, input.sessionStatusEventsPath),
...
const stableSessionFields = (artifact) => ({
    ...
    active_checkpoint_response_path: artifact.active_checkpoint?.response_path ?? null,
    latest_round: artifact.latest_round ?? null,
    latest_stop_reason: artifact.latest_stop_reason ?? null
});
const diffSessionStatusFields = (previous, next) => {
    const nextStable = stableSessionFields(next);
    if (!previous) {
        return Object.keys(nextStable);
    }
    const previousStable = stableSessionFields(previous);
    return Object.keys(nextStable).filter((field) => previousStable[field] !== nextStable[field]);
};
const buildSessionStatusEventArtifact = async (input) => {
    const changedFields = diffSessionStatusFields(input.previousSessionStatus, input.nextSessionStatus);
    if (changedFields.length === 0) {
        return undefined;
    }
    const existingEvents = await loadJsonLinesIfExists(input.sessionStatusEventsPath);
...
export const buildSessionStreamContractArtifact = (input) => ({
    ...
    preferred_delivery: input.transportMode === "app-server" && input.appServerSessionEventsPath
        ? "app_server_notification_jsonl"
        : "file_tail_jsonl",
    snapshot_path: relativeToRun(input.runDirectory, input.sessionStatusPath),
    source_events_path: relativeToRun(input.runDirectory, input.sessionStatusEventsPath),
    ...(input.appServerSessionEventsPath
        ? {
            app_server_events_path: relativeToRun(input.runDirectory, input.appServerSessionEventsPath)
        }
        : {}),
...
const preparedIdeaRawMarkdown = (input) => [
    ...
    "## Quality Bar",
    "",
    ...input.buildBrief.product.success_definition.map((criterion) => `- ${criterion}`),
    "",
    "## Constraints",
    "",
    ...unique([
...
export const loadPreparedSessionSeed = async (input) => {
    const [buildBrief, runContract] = await Promise.all([
        loadJsonIfExists(input.buildBriefPath),
        loadJsonIfExists(input.runContractPath)
    ]);
    ...
    const idea = {
        ...
        user_goals: buildBrief.product.core_workflows.length > 0
        ...
        quality_bar: buildBrief.product.success_definition.length > 0
            ? [...buildBrief.product.success_definition]
    ...
    return {
        ...
        durableMemory: {
            ...
            qualityBar: buildBrief.product.success_definition.length > 0
                ? [...buildBrief.product.success_definition]
                : [...runContract.stop_rule.done_when],
            constraints: unique([
                ...buildBrief.constraints.stack_preferences,
...
const executionPlanMarkdown = (input) => [
    ...
    "",
    bulletList(input.scenario.acceptance_highlights, "No acceptance highlights were recorded yet."),
    "",
    "## Controller Strategy",
    "",
    ...
    "",
    bulletList(input.plan.planner_acceptance_checks, "No planner acceptance checks were recorded."),
    "",
    "## Remediation Policy",
    "",
    bulletList(input.plan.remediation_policy, "No remediation policy was recorded."),
    "",
    "## Live Review Context",
    "",
    bulletList(input.openQuestions.review_feedback, "none"),
    "",
    "## Steering Context",
    "",
    bulletList(input.openQuestions.steering_notes, "none"),
    "",
    "## External Blockers",
    "",
    bulletList(input.openQuestions.external_blockers, "none"),
    "",
    "## Deferred Questions",
    "",
    bulletList(input.openQuestions.questions.map((question) => question.prompt), "none"),
    ""
].join("\n");
export const writeSessionPreparationArtifacts = async (input) => {
    const now = new Date().toISOString();
    const intake = await loadJsonIfExists(join(input.rootDirectory, "intake.json"));
    const existingBuildBrief = await loadJsonIfExists(input.buildBriefPath);
    const existingRunContract = await loadJsonIfExists(input.runContractPath);
    const existingSessionStatus = await loadJsonIfExists(input.sessionStatusPath);
    const targetManifestHints = buildTargetManifestHints(intake);
    const isProductBuild = Boolean(input.discoverySource || intake?.product_title || intake?.target_family);
    ...
    const deliveryLevel = intake?.goal_level ?? inferGoalLevelFromTargetScore(intake?.target_score ?? input.plan.target_total_score);
    ...
    const targetUsers = intake?.target_users && intake.target_users.length > 0
        ? unique(intake.target_users)
        : isProductBuild
            ? []
            : input.durableMemory.targetUsers;
    const coreWorkflows = intake?.core_features && intake.core_features.length > 0
    ...
    const objective = input.currentObjective ??
        (primaryFinishLine
    ...
    const buildBrief = {
        brief_id: existingBuildBrief?.brief_id ??
    ...
    const runContract = {
        contract_id: existingRunContract?.contract_id ??
    ...
    if (input.discoverySource || intake?.product_title || intake?.target_family) {
        ...
        if (integrityErrors.length > 0) {
            throw new Error(`Prepared product session failed integrity checks:\n${integrityErrors
                .map((error) => `- ${error}`)
                .join("\n")}`);
        }
    }
    await Promise.all([
        ...
        writeText(input.executionPlanPath, executionPlanMarkdown({
            runId: input.runId,
            title: buildBrief.product.title,
            summary: buildBrief.product.summary,
            workspaceMode: input.workspaceMode,
            ...
            sessionStatusEventsPath: relativeToRun(input.runDirectory, input.sessionStatusEventsPath),
    ...
    const latestSourceSequence = sessionStatusEvent?.sequence ??
        (await loadJsonLinesIfExists(input.sessionStatusEventsPath)).at(-1)?.sequence;
