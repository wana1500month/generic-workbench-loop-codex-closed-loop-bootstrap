...
const dimensionScoreList = (dimensions) => dimensions.length > 0
    ? dimensions
        .map((dimension) => `- ${dimension.label}: ${dimension.score.toFixed(3)} / min ${dimension.minimum_score.toFixed(3)} (${dimension.passed ? "pass" : dimension.applicable ? "fail" : "n/a"})`)
        .join("\n")
    : "- none";
export const plannerBriefPathForRun = (runDirectory) => join(runDirectory, "planner-brief.md");
...
${bulletList(input.qualityCritique.findings.map((finding) => `${finding.category}/${finding.severity}: ${finding.expected_change}`))}
...
export const writeRunControllerSummary = async (input) => {
    ...
    const latestRound = input.summary.round_history?.[input.summary.round_history.length - 1];
    await writeText(path, `# Controller Summary
