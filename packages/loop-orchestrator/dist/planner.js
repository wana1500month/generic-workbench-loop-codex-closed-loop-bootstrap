...
const rankFocusAreas = (idea) => {
    const text = normalizedIdeaText(idea);
    return focusAreaSignals
        .map(({ focusArea, baseScore, patterns }) => ({
        focusArea,
        score: baseScore +
            patterns.reduce((total, pattern) => total + (pattern.test(text) ? 2 : 0), 0)
    }))
        .sort((left, right) => {
        if (right.score !== left.score) {
            return right.score - left.score;
        }
        return (focusAreaPriority.indexOf(left.focusArea) -
            focusAreaPriority.indexOf(right.focusArea));
    })
        .slice(0, maxPlannerFocusAreas)
        .map((entry) => entry.focusArea);
};
const checksForFocusAreas = (focusAreas) => unique(focusAreas.flatMap((focus) => {
    switch (focus) {
...
export const buildAttemptDirective = (input) => {
    const carryOverCheckIds = unique(input.previousPatchRequest?.must_fix.flatMap((item) => item.target_check_ids) ?? []);
    const thresholdOnly = carryOverCheckIds.length > 0 &&
    ...
    return {
        ...
        objective: carryOverCheckIds.length > 0
            ? remediationObjective
            : initialObjective,
        focus_areas: input.plan.planner_focus_areas,
        rewrite_scope: remediationAttempt ? "incremental" : "integration",
        acceptance_checks: carryOverCheckIds.length > 0 ? carryOverCheckIds : input.plan.planner_acceptance_checks
    };
};
...
export const buildRoundContract = (input) => {
    ...
    const carryOverCheckIds = unique(input.previousPatchRequest?.must_fix.flatMap((item) => item.target_check_ids) ?? []);
    const productBuild = /runtime\/build-brief\.json|release-gate workflow probes/i.test(input.directive.objective);
    const briefNote = input.directive.attempt_kind === "remediation"
