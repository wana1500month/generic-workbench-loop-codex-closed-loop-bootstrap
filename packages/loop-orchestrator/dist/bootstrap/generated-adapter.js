...
import { applyChangeTemplate, captureEvidenceTemplate, gradeRoundTemplate, helperTemplate, moduleImportPath, prepareTargetTemplate, runChecksTemplate, runTargetTemplate } from "./templates.js";
...
const workflowRootSelector = (index) => `[data-workflow-id='workflow-${index + 1}'], [data-testid='feature-${index + 1}']`;
const workflowActionSelector = (index) => `[data-workflow-id='workflow-${index + 1}'] [data-workflow-action='primary'], [data-testid='feature-${index + 1}-action']`;
const workflowResultSelector = (index) => `[data-workflow-id='workflow-${index + 1}'] [data-workflow-result='primary'], [data-testid='feature-${index + 1}-result']`;
const featureProbeMap = (features) => features.slice(0, 3).map((feature, index) => {
...
const workflowProbeMap = (input) => {
    const checks = input.workflowChecks.length > 0
    ...
    return checks.slice(0, 5).map((check, index) => {
        ...
        return {
            ...
            action_selector: check.selectorHints?.action ?? workflowActionSelector(index),
            result_selector: check.selectorHints?.result ?? workflowResultSelector(index),
            legacy_root_selector: `[data-testid='feature-${index + 1}']`,
            legacy_action_selector: `[data-testid='feature-${index + 1}-action']`,
            legacy_result_selector: `[data-testid='feature-${index + 1}-result']`,
...
const adapterReviewTask = (answers, paths) => [
    ...
    "",
    "Workflow checks:",
    ...answers.workflowChecks.map((check, index) => `${index + 1}. ${check.workflow} -> ${check.expectedResult} (${check.surface})`)
].join("\n");
export const scaffoldAdapterArtifacts = async (answers, paths) => {
    const runtimeConfig = {
        ...
        verification_contract: {
            ...
            finish_line_selector: "[data-harness='finish-line-ready'], [data-testid='finish-line-ready']",
    ...
    const codexRuntimeImportPath = moduleImportPath(paths.generatedScriptsRoot, join(repoRoot, "packages", "loop-orchestrator", "dist", "codex-runtime.js"));
    await writeJson(paths.generatedRuntimeConfigPath, runtimeConfig);
    await writeText(join(paths.generatedScriptsRoot, "runtime-helpers.mjs"), helperTemplate(codexRuntimeImportPath));
    ...
    await writeJson(paths.adapterReviewResponsePath, {
        status: "not_reviewed",
        instructions: "Review adapter-review-task.md before loop start if the operator wants an explicit generated-adapter review checkpoint.",
...
//# sourceMappingURL=generated-adapter.js.map