...
import { loadJson, loadJsonIfExists, pathExists, repoRoot, writeJson } from "./file-system.js";
import { resumeIdentityFingerprint } from "./resume-identity.js";
const targetManifestKeys = new Set([
    "app_url",
    "health_url",
    "api_base_url"
]);
const fileSafeToken = (value) => value.replace(/[<>:"/\\|?*\x00-\x1f]+/g, "-");
const normalizeString = (value) => typeof value === "string" && value.trim().length > 0 ? value.trim() : undefined;
const boundaryBreakBlockerPattern = /(verification provider|provider[_\s-]?id|required capability|missing capability|contract version|target root|adapter id)/i;
const normalizePatchPath = (value) => value.replaceAll("\\", "/");
const isGeneratedLocalAdapterSurfacePath = (relativePath) => {
    ...
    return (normalized === "adapter.generated.json" ||
...
const parsePatchChangedFiles = (patchText) => {
...
const runGitApply = async (input) => new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("close", (code) => {
        ...
        rejectPromise(new Error(`git apply failed for '${input.patchPath}' in '${input.cwd}'.${stdout.trim() ? `\n${stdout.trim()}` : ""}${stderr.trim() ? `\n${stderr.trim()}` : ""}`));
    });
});
export const generatedAdapterRuntimeConfigPath = (adapterContractPath) => basename(dirname(adapterContractPath)) === "generated-adapter"
...
export const detectAdapterOrigin = (loadedAdapter) => loadedAdapter &&
    basename(loadedAdapter.contract_path) === "adapter.generated.json" &&
    loadedAdapter.contract.adapter_id.startsWith("generated-")
    ? "generated_local"
    : "external_contract";
const inferRuntimeSurfaceValue = (key, runtimeConfig) => {
    switch (key) {
        case "app_url":
            return normalizeString(runtimeConfig.app_url) ?? normalizeString(runtimeConfig.ready_url);
        case "health_url":
            return (normalizeString(runtimeConfig.health_url) ?? normalizeString(runtimeConfig.ready_url));
        case "api_base_url":
            return (normalizeString(runtimeConfig.api_base_url) ?? normalizeString(runtimeConfig.ready_url));
        default:
            return undefined;
    }
};
const classifyAdapterMigration = (input) => {
    if (input.adapterOrigin === "generated_local" &&
    ...
    const boundaryBreakDetected = (input.adapterOrigin === "external_contract" &&
        input.adapterDriftReport.kind === "contract") ||
        input.adapterDriftReport.static_blockers.some((blocker) => boundaryBreakBlockerPattern.test(blocker));
    ...
    return {
        migrationClass: input.adapterDriftReport.kind === "contract"
            ? "kernel_wiring_patch"
            : "runtime_surface_patch",
        applyMode: input.adapterOrigin === "generated_local" &&
            input.adapterDriftReport.kind === "contract"
            ? "same_run_in_place"
            : "proposal_only",
        sameRunEligible: input.adapterOrigin === "generated_local" &&
            input.adapterDriftReport.kind === "contract",
        autoapplyEligible: false,
        requiresOperatorAcceptance: true,
        forceNewRun: false
...
export const decisionOptionsForAdapterMigrationProposal = (proposal) => proposal.force_new_run
    ? ["open_new_run", "reject"]
    : ["accept", "reject", "open_new_run"];
export const approvalSemanticsForAdapterMigrationProposal = (proposal) => ({
    accept: proposal.same_run_eligible && proposal.patch_bundle_path
    ...
    reject: "Reject the migration proposal and close the current run with adapter_migration_rejected.",
    open_new_run: proposal.force_new_run
...
export const buildAdapterMigrationProposal = async (input) => {
    ...
    const runtimeConfigPath = adapterOrigin === "generated_local"
        ? generatedAdapterRuntimeConfigPath(input.loadedAdapter.contract_path)
        : undefined;
    const runtimeConfig = runtimeConfigPath
        ? await loadJsonIfExists(runtimeConfigPath)
        : undefined;
    const missingKeys = input.adapterDriftReport.missing_target_manifest_keys.filter((key) => targetManifestKeys.has(key));
    const inferredPatchEntries = missingKeys
        .map((key) => [key, inferRuntimeSurfaceValue(key, runtimeConfig ?? {})])
        .filter((entry) => Boolean(entry[1]));
    const inferableKeys = inferredPatchEntries.map(([key]) => key);
    const proposedRuntimeConfigPatch = inferredPatchEntries.length > 0
    ...
    const runtimeConfigRelativePath = runtimeConfigPath && runtimeConfigPath.startsWith(dirname(input.loadedAdapter.contract_path))
    ...
    return {
        ...
        summary: classification.autoapplyEligible && proposedRuntimeConfigPatch
            ? `Generated adapter runtime surface drift can be repaired in-place by updating ${Object.keys(proposedRuntimeConfigPatch).join(", ")} before the recontract round continues.`
...
export const applyGeneratedLocalAdapterMigration = async (input) => {
    if (input.proposal.adapter_origin !== "generated_local" ||
        input.proposal.apply_mode !== "same_run_in_place") {
        throw new Error(`Adapter migration proposal '${input.proposal.proposal_id}' is not eligible for same-run generated-local apply.`);
    }
    ...
    const generatedAdapterRoot = basename(adapterRoot) === "generated-adapter"
        ? resolve(adapterRoot, "codex-adapter")
        : resolve(adapterRoot, ".generated", "codex-adapter");
    const backupDirectory = join(input.runtimeDirectory, "adapter-migrations", fileSafeToken(input.proposal.proposal_id), "before");
    ...
    if (input.proposal.patch_bundle_path) {
        ...
        if (patchChangedFiles.length === 0) {
            throw new Error(`Adapter migration patch bundle '${patchPath}' did not declare any changed files.`);
        }
        const invalidPath = patchChangedFiles.find((relativePath) => !isGeneratedLocalAdapterSurfacePath(relativePath));
        if (invalidPath) {
            throw new Error(`Adapter migration patch bundle '${patchPath}' touched '${invalidPath}', which is outside the generated adapter write surface.`);
        }
        await runGitApply({
            ...
            ...(isPathInside(repoRoot, adapterRoot)
                ? {
                    directory: normalizePatchPath(relative(repoRoot, adapterRoot)).replace(/^$/, ".")
                }
                : {})
        });
    ...
    if (input.proposal.proposed_runtime_config_patch) {
        const runtimeConfig = await loadJson(runtimeConfigPath);
    ...
    const adapterContract = await loadJson(adapterContractPath);
    const currentNotes = Array.isArray(adapterContract.notes)
        ? adapterContract.notes.filter((note) => typeof note === "string")
        : [];
    const migrationNote = input.proposal.proposed_contract_patch &&
...
const isAdapterMigrationDecision = (value) => value === "accept" || value === "reject" || value === "open_new_run";
export const loadAdapterMigrationResponse = async (path) => {
    const artifact = await loadJsonIfExists(path);
    if (!artifact) {
        return undefined;
    }
...
export const loadAuthorizedAdapterMigration = async (summary) => {
    const explicitPath = summary?.adapter_migration_applied_path;
    if (explicitPath) {
        return loadJsonIfExists(explicitPath);
    }
    ...
    return latestRoundPath
        ? loadJsonIfExists(latestRoundPath)
        : undefined;
};
export const isAuthorizedAdapterMigration = (input) => {
    ...
    return (resumeIdentityFingerprint(input.previousIdentity) ===
...
//# sourceMappingURL=adapter-migration.js.map