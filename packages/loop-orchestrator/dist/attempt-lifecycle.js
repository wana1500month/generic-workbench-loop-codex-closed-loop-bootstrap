...
const recontractReasonForTrigger = (triggerCode, remediationHistory) => {
    switch (triggerCode) {
...
const recontractReasonForAdapterDrift = (kind) => kind === "contract" ? "adapter_contract_drift" : "adapter_runtime_drift";
export const targetCheckIdsFromPatchRequest = (patchRequest) => unique(patchRequest?.must_fix.flatMap((item) => item.target_check_ids).filter(Boolean) ?? []);
export const unresolvedSignatureFor = (unresolvedCheckIds) => {
    const normalized = unique(unresolvedCheckIds).sort();
    return normalized.length > 0 ? normalized.join("|") : undefined;
};
export const decideAttemptLifecycle = (input) => {
    const trajectory = input.previousTrajectoryDecision ??
    ...
    if (input.previousPatchRequest?.next_action === "recontract_adapter") {
        return {
            ...
            reason: input.previousPatchRequest.adapter_drift_summary ??
            ...
            recontract_reason: recontractReasonForAdapterDrift(input.previousPatchRequest.adapter_drift_kind),
            trajectory
        };
    }
    ...
    if (trajectory.mode === "pivot" || trajectory.mode === "parallel_pivot") {
        return {
            ...
            recontract_reason: recontractReasonForTrigger(input.remediationHistory?.policy_snapshot?.dominant_trigger_code, input.remediationHistory),
            trajectory
        };
    }
    if ((input.remediationHistory?.target_manifest_keys_missing.length ?? 0) > 0) {
        return {
            ...
            reopen_contract: true,
            decision_source: "hard_rule",
            reason: `Release-gate target manifest keys are still missing: ${input.remediationHistory?.target_manifest_keys_missing.join(", ")}.`,
    ...
    if (input.remediationHistory?.scope_drift_detected) {
        return {
            ...
            reopen_contract: true,
            decision_source: "hard_rule",
            reason: "The latest patch request widened scope beyond the active contract frame, so remediation should re-contract before continuing.",
    ...
    if (input.remediationHistory?.policy_snapshot?.recommended_action === "recontract") {
        ...
        return {
            ...
            recontract_reason: recontractReasonForTrigger(dominantTrigger, input.remediationHistory),
            trajectory
        };
    }
    return {
        ...
        reason: "An active contract frame exists and the latest patch request has actionable check ids, so remediation stays patch-only.",
        trajectory
    };
};
...
//# sourceMappingURL=attempt-lifecycle.js.map