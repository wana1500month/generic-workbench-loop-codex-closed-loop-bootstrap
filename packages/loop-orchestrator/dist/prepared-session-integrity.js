...
export const validatePreparedProductSessionIntegrity = (input) => {
    ...
    if (/workbench|controller|adapter/i.test(JSON.stringify(input.buildBrief.product.success_definition))) {
        errors.push("Product success_definition appears contaminated by harness quality-bar language.");
    }
    ...
    if (buildTargetRoot &&
        ...
        errors.push(`buildBrief target_root and runContract target_root disagree: ${buildTargetRoot} vs ${contractTargetRoot}`);
    }
    const targetRoot = contractTargetRoot;
    if (/^(?:income\/expense|\/지출|수입\/지출|로그인\/회원가입|A\/B)$/u.test(targetRoot)) {
        errors.push(`target_root looks like a workflow phrase, not a project path: ${targetRoot}`);
    }
...
//# sourceMappingURL=prepared-session-integrity.js.map