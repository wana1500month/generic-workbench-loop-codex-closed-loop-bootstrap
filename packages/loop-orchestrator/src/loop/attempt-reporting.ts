...
import {
  externalBlockersFromPatchRequest,
  reviewFeedbackFromArtifacts,
  scopeGuardrailsFromPatchRequest
} from "./runtime-warning-summary.js";
import {
  stopReasonForRoundTargetDecision,
  type RoundTargetDecisionState
} from "./round-target-decision.js";

type AttemptLifecycleDecision = ReturnType<typeof decideAttemptLifecycle>;

type WithPhaseBudget = <T>(
...
export const buildAttemptRoundReport = (input: {
  ...
  scoreDeltas: number[];
  adapterMigrationStopPreview?: { apply_mode: string };
  ...
  const latestRoundState: RoundTargetDecisionState = {
    ...
    staticAdapterContractInvalid:
      input.contractReviewArtifact.static_blockers.length > 0 &&
  ...
  const unresolvedSignature = unresolvedSignatureFor(
    input.roundResultArtifact.unresolved_check_ids
  );
...
export const commitAttemptRoundReport = async (input: {
  ...
  input.withPhaseBudget("round_commit", async () => {
    const latestRoundSummaryPath = join(
