...
export const runEvaluatorStep = async (input: {
  resumedRoundPhase:
    | { phase: ControllerRoundPhase; status: ControllerPhaseStatus }
    | undefined;
  ...
  if (phaseCompletedAtOrBeyond(input.resumedRoundPhase, "evaluation")) {
    const evalReport = await loadJson<EvalReport>(input.artifacts.eval_report_path);
    const roundResultArtifact = await loadJson<RoundResultArtifact>(
      input.artifacts.round_result_json_path
    );
    return {
      evalReport,
      ...
      qualityCritiqueArtifact: await loadJson<QualityCritiqueArtifact>(
        input.artifacts.quality_critique_json_path
      ),
      ...
      trajectoryDecisionArtifact: await loadJson<TrajectoryDecisionArtifact>(
        input.artifacts.trajectory_decision_json_path
      ),
      roundResultArtifact,
      roundScorecard: await loadJsonIfExists<RoundScorecard>(
        input.artifacts.scorecard_json_path
      ),
      failureLineage:
        (await loadJsonIfExists<FailureLineage>(
          input.artifacts.failure_lineage_path
        )) ??
      ...
      adapterDriftReport: await loadJsonIfExists(
        input.artifacts.adapter_drift_report_json_path
      ),
  ...
  let output: Exclude<EvaluatorStepResult, { checkpointResult: ClosedLoopResult }>;
  const evaluationResult = await input.withPhaseBudget(
    "evaluation",
    async (): Promise<ClosedLoopResult | undefined> => {
      ...
      const resolvedEvalEnhancement =
        input.transportMode === "app-server" && input.appServerTransport
      ...
      const projectedScoreDeltas =
        input.history.length > 0
          ? [
              ...
              Number(
                (
                  evalReport.total_score -
                  input.history[input.history.length - 1].total_score
                ).toFixed(3)
              )
            ].slice(-6)
          : input.scoreDeltas.slice(-6);
      const projectedPlateauCount = isImproved(
        evalReport.total_score,
        input.bestScore
      )
        ? 0
      ...
      const adapterMigrationStopPreview =
        adapterDriftReport && input.loadedAdapter
          ? await buildAdapterMigrationProposal({
              runId: input.runId,
              round: input.round + 1,
              sourceAdapterDriftReportPath:
                input.artifacts.adapter_drift_report_json_path,
              loadedAdapter: input.loadedAdapter,
              adapterDriftReport
            })
      ...
      await input.markProgress(
        `Evaluation artifacts saved for round ${input.round}.`
      );
      await input.recordRoundPhase({
        round: input.round,
        phase: "evaluation",
