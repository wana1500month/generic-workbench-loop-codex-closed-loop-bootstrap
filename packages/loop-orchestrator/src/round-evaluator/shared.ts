...
export const adapterRuntimePath = join(
  repoRoot,
  "packages",
  "loop-orchestrator",
  "src",
...
export const placeholderSurfaceChecks = new Set<string>([
...
export const artifactOnlyChecks = new Set<string>([
...
export const knownCheckIds = new Set<string>([
  ...artifactOnlyChecks,
...
export const nonCarryForwardDerivedChecks = new Set<string>([
  "previous_patch_request_addressed",
  "previous_patch_request_resolved"
]);
export const proofEvaluatorChecks = new Set<string>([
  "adapter_execution_healthy",
  "adapter_claims_are_honest",
  "proof_provenance_is_attested",
  "live_verification_present",
...
export const nonScoringDerivedChecks = new Set<string>([
  "target_signal_thresholds_met",
  "previous_patch_request_addressed",
  "previous_patch_request_resolved"
]);
...
export const isProofCapabilityName = (value: string): boolean => proofCapabilityKinds.has(value);

export const proofExecutionsFor = (
...
export const buildProofEvidenceOriginIndex = (
  adapterExecutions: readonly AdapterCapabilityExecution[]
...
export const observedValueMatches = (
...
export const isPassingCheck = (result: RoundCheckResult): boolean => result.status === "pass";
export const isFailingCheck = (result: RoundCheckResult): boolean => result.status === "fail";
export const isSatisfiedCheck = (result: RoundCheckResult): boolean => result.status !== "fail";
export const isApplicableCheck = (result: RoundCheckResult): boolean =>
  result.status !== "not_applicable";

export const ratioScore = (passedItems: number, totalItems: number): number =>
  totalItems === 0 ? 0 : passedItems / totalItems;

export const strictPartialCreditScore = (passedItems: number, totalItems: number): number =>
  totalItems === 0
    ? 0
    : Number(Math.pow(ratioScore(passedItems, totalItems), 2).toFixed(3));

export const scoreFromResults = (
...
export const isKnownCheck = (checkId: string): boolean => knownCheckIds.has(checkId);

export const pathExists = (path?: string): boolean => (path ? existsSync(path) : false);
export const requiredProofCapabilities = ["capture_evidence", "run_checks", "grade_round"] as const;

export const requiredCoreProbesFor = (loadedAdapter?: LoadedAdapterContract) =>
...
export const coreProbeRole = (probe: VerificationCoreProbe) =>
  probe.role ?? (releaseGateCoreProbeModes.has(probe.mode) ? "release_gate" : "supporting");

export const probeSemanticLevel = (probe: VerificationCoreProbe) =>
  probe.semantic_level ??
  (releaseGateCoreProbeModes.has(probe.mode) ? "feature" : "liveness");

export const assertionIdForCriterion = (criterion: VerificationCriterion): string =>
  criterion.assertion_id?.trim() || criterion.criterion_id;

export const releaseAssertionIdForProbe = (
...
export const requiredReleaseGateCoreProbesFor = (loadedAdapter?: LoadedAdapterContract) =>
  requiredCoreProbesFor(loadedAdapter).filter(
    (probe) =>
      coreProbeRole(probe) === "release_gate" && releaseGateCoreProbeModes.has(probe.mode)
  );

export const requiredBrowserJourneyReleaseProbesFor = (loadedAdapter?: LoadedAdapterContract) =>
  requiredReleaseGateCoreProbesFor(loadedAdapter).filter(
    (probe) => probe.mode === "browser_journey"
  );

export const requiredHttpJsonReleaseProbesFor = (loadedAdapter?: LoadedAdapterContract) =>
  requiredReleaseGateCoreProbesFor(loadedAdapter).filter(
    (probe) => probe.mode === "http_json"
  );

export const minimumFeatureReleaseAssertionsFor = (loadedAdapter?: LoadedAdapterContract): number =>
  loadedAdapter?.verification_profile?.profile.minimum_feature_release_assertions ?? 2;

export const minimumAssertionTagCountsFor = (
...
export const expectedTargetSurfacesFor = (loadedAdapter?: LoadedAdapterContract): Set<"browser" | "api"> =>
  new Set(loadedAdapter?.verification_profile?.profile.expected_target_surfaces ?? []);

export const normalizedWeights = <T extends string>(
...
export const proofScoreWeightsFor = (loadedAdapter?: LoadedAdapterContract) => {
...
export const releaseScoreWeightsFor = (loadedAdapter?: LoadedAdapterContract) =>
  normalizedWeights(
...
export const isVisualEvidencePath = (path: string): boolean => {
...
export const successfulGradeRoundExecutionFor = (
  adapterExecutions: readonly AdapterCapabilityExecution[]
...
export const assertionTagLabel = (tag: VerificationAssertionTag): string => {
  switch (tag) {
    case "browser":
      return "browser";
    case "api":
...
export const configuredReleaseAssertionIdsForTag = (
...
export const releaseGateAssertionIdsFor = (loadedAdapter?: LoadedAdapterContract): Set<string> =>
  new Set(
...
export const hardReleaseAssertionIdsFor = (loadedAdapter?: LoadedAdapterContract): Set<string> => {
...
export const verificationBoundaryIssues = (
  loadedAdapter?: LoadedAdapterContract
): string[] => {
  if (!loadedAdapter) {
    return [];
...
export const fileSurfaceReservedCheck = (
  check_id: string,
  path: string | undefined,
  label: string
): RoundCheckResult =>
