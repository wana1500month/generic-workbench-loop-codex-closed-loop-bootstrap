...
const main = async () => {
  ...
  const witness = {
    ...
    assertion_ids: ["target_accessible"],
    steps: [
      {
  ...
  const criteriaResults = [
    {
      ...
      hard: true,
      threshold: "The ready URL should answer while the target is running.",
      observed_value: liveProbe.ok ? "HTTP " + liveProbe.status : "No successful response",
      evidence_paths: [interactionLogPath, witnessPath]
    },
    {
      criterion_id: "command_checks",
      status: checksOk ? "pass" : "fail",
      summary: checkSummary,
      hard: false,
      threshold: "Configured check command should exit successfully when present.",
      observed_value: config.check_command
        ? (checksOk ? "check command passed" : "check command failed")
        : "no check command configured",
      evidence_paths: checkLogPath ? [checkLogPath] : [interactionLogPath]
    }
  ];

  const evidencePaths = [interactionLogPath, witnessPath, ...(checkLogPath ? [checkLogPath] : [])];
  await finalize({
    capability: "run_checks",
    ok: liveProbe.ok && checksOk,
    summary:
      liveProbe.ok && checksOk
        ? "Live probe and configured checks passed."
        : "Live probe or configured checks failed.",
    findings:
      liveProbe.ok && checksOk
        ? []
        : [
            !liveProbe.ok ? "Ready URL probe failed." : undefined,
            !checksOk ? "Configured check command failed." : undefined
          ].filter(Boolean),
    evidence_paths: evidencePaths,
    evidence_items: [
      {
...
const main = async () => {
  ...
  const checksOk = checksResult?.ok === true;
  const releaseScore = checksOk ? 0.82 : 0.38;
  const thresholdVerdict = checksOk ? "pass" : "fail";
  const overallVerdict = checksOk ? "advance" : "revise";
  const reportPath = await writeArtifact(
    "grade-summary.md",
    [
      "# Round grading",
      "",
      "Checks OK: " + String(checksOk),
  ...
  await finalize({
    ...
    criteria_results: Array.isArray(checksResult?.criteria_results)
      ? checksResult.criteria_results
      : [],
    score: releaseScore,
    overall_verdict: overallVerdict,
    threshold_verdict: thresholdVerdict,
    blocking_criterion_ids: checksOk ? [] : ["target_accessible"]
  });
};
