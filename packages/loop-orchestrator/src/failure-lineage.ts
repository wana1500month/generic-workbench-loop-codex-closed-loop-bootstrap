...
): VerificationCoreProbe["role"] =>
  mode === "http_json" || mode === "browser_journey" ? "release_gate" : "supporting";

const failingAssertionIdsFor = (evalReport: EvalReport): string[] => {
  const criterionAssertionIds = evalReport.adapter_results.flatMap((execution) =>
    execution.verified_criteria_results
      .filter((criterion) => criterion.status === "fail")
      .map((criterion) => criterion.criterion_id)
  );
  const failingProbeAssertionIds = evalReport.core_probe_results.flatMap((probe) =>
    !probe.ok && probe.assertion_id ? [probe.assertion_id] : []
  );

  return unique([...criterionAssertionIds, ...failingProbeAssertionIds]);
};

const contradictoryWitnessAssertionIdsFor = (evalReport: EvalReport): string[] => {
  const witnessedAssertionIds = unique(
    evalReport.adapter_results.flatMap((execution) =>
      execution.verified_evidence.flatMap((evidence) => evidence.witness?.assertion_ids ?? [])
    )
  );
  const expectedAssertionIds = new Set(
    unique([
      ...
      ...evalReport.core_probe_results.flatMap((probe) =>
        probe.assertion_id ? [probe.assertion_id] : []
      )
    ])
  );
...
const missingTargetManifestKeysFor = (input: {
  loadedAdapter?: LoadedAdapterContract;
  targetManifest?: TargetManifest;
}): string[] =>
  unique(
    input.loadedAdapter?.verification_profile?.profile.core_probes?.flatMap((probe) => {
...
export const buildFailureLineageArtifact = (input: {
  ...
  const environmentBlockedProbeIds = input.evalReport.core_probe_results
    ...
    .map((probe) => probe.probe_id);
  const environmentBlockedAssertionIds = new Set(
    input.evalReport.core_probe_results.flatMap((probe) =>
      !probe.ok &&
      probe.role === "release_gate" &&
      probe.failure_classification === "environment_blocked" &&
      probe.assertion_id
  ...
  const contradictoryWitnessAssertionIds = contradictoryWitnessAssertionIdsFor(
    input.evalReport
  );
  ...
  const releaseRegressionIds =
    input.previousRoundSummary?.resolved_check_ids.filter((checkId) =>
      input.evalReport.unresolved_check_ids.includes(checkId)
    ) ?? [];
  const hasAnyFailures =
    input.evalReport.unresolved_check_ids.length > 0 ||
  ...
  const hasOnlyEnvironmentBlockedReleaseProbes =
    failingProbeIds.length > 0 &&
    environmentBlockedProbeIds.length === failingProbeIds.length &&
    failingAssertionIds.every((assertionId) =>
      environmentBlockedAssertionIds.has(assertionId)
    ) &&
    missingTargetManifestKeys.length === 0 &&
    contradictoryWitnessAssertionIds.length === 0 &&
    releaseRegressionIds.length === 0;
...
const repeatedCountFromHistory = <T>(
  history: readonly RoundSummary[],
  currentValue: T | undefined,
  selector: (round: RoundSummary) => T | undefined,
  allowNone = false
): number => {
  if (currentValue === undefined) {
    return 0;
  }
  if (!allowNone && currentValue === ("none" as T)) {
    return 0;
  }
...
export const buildFailureLineagePolicySnapshot = (input: {
  ...
  const repeatedFailureSignatureCount = repeatedCountFromHistory(
    input.history,
    input.failureLineage.unresolved_signature,
    (round) => round.failure_lineage?.unresolved_signature ?? round.failure_lineage?.unresolved_signature
  );
  const repeatedFailureClassificationCount = repeatedCountFromHistory(
    input.history,
    input.failureLineage.failure_classification,
  ...
  const missingManifestCount =
    input.failureLineage.missing_target_manifest_keys.length;
  const environmentBlocked =
    input.failureLineage.failure_classification === "environment_blocked";
  const patchEntropySpike = input.patchEntropy >= 2;
  const triggerScores: Partial<Record<FailureLineageTriggerCode, number>> = {
    environment_blocked: environmentBlocked ? 1 : 0,
    manifest_contract_broken:
      missingManifestCount > 0 ? Math.min(1, 0.8 + missingManifestCount * 0.05) : 0,
    release_gate_regression:
      regressionCount > 0 ? Math.min(1, 0.75 + regressionCount * 0.05) : 0,
    scope_drift: input.scopeDriftDetected ? 0.72 : 0,
    contradiction_detected:
      contradictionCount > 0 ? Math.min(1, 0.7 + contradictionCount * 0.08) : 0,
  ...
  const triggerEntries = Object.entries(triggerScores) as Array<
    [FailureLineageTriggerCode, number]
  >;
  ...
  const [dominantTriggerCode, dominantTriggerScore] =
    dominantWeightedTrigger && dominantWeightedTrigger[1] > 0
      ? dominantWeightedTrigger
      : ([
          "stable_patch_authority",
          triggerScores.stable_patch_authority ?? 0.2
        ] as [FailureLineageTriggerCode, number]);
  let patchAuthorityState: PatchAuthorityState = "healthy";
  if (
    environmentBlocked ||
  ...
  let recommendedAction: FailureLineagePolicySnapshot["recommended_action"] =
    "patch_only";
  let recommendationSource: FailureLineagePolicySnapshot["recommendation_source"] =
    "weighted_policy";
  ...
  if (environmentBlocked) {
    ...
    reasons.push(
      "Latest release blockers are environment-blocked only, so bounded reopen would waste remediation budget."
    );
    ...
    reasons.push(
      "Active contract frame is still credible, so remediation can stay patch-only."
    );
  }
...
export const isPureEnvironmentBlockedLineage = (
  failureLineage?: FailureLineage
): boolean =>
  Boolean(
    failureLineage &&
...
export const failureLineageArtifactPath = (roundDirectory: string): string =>
  join(roundDirectory, "failure-lineage.json");

export const loadFailureLineageArtifact = async (
  path?: string
): Promise<FailureLineage | undefined> => {
  if (!path) {
    return undefined;
  }
  ...
  return loadJson<FailureLineage>(path);
};
