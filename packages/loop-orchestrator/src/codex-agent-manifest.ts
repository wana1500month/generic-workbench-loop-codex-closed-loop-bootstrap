...
export const experimentalExecutorRuntimeWarning =
  "Executor mode 'subagents-experimental' uses manifest-backed prompt orchestration and still executes one Codex CLI call per harness stage.";

const manifestCache = new Map<CodexAgentRole, CodexAgentManifest | null>();

const parseQuotedField = (raw: string, key: string): string | undefined =>
  raw.match(new RegExp(`^${key}\\s*=\\s*"([^"\\r\\n]+)"`, "m"))?.[1]?.trim() || undefined;

const parseMultilineField = (raw: string, key: string): string | undefined =>
  raw
    .match(new RegExp(`^${key}\\s*=\\s*"""\\r?\\n([\\s\\S]*?)\\r?\\n"""`, "m"))?.[1]
    ?.trim() || undefined;

const loadCodexAgentManifest = async (
  role: CodexAgentRole
): Promise<CodexAgentManifest | null> => {
  ...
  try {
    ...
    const manifest: CodexAgentManifest = {
      path,
...
export const buildExecutorModePrompt = async (input: {
  ...
  role: CodexAgentRole;
  prompt: string;
}): Promise<{ prompt: string; warning?: string; manifestPath?: string }> => {
