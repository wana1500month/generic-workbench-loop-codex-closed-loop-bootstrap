...
const targetCheckIdsFromPatch = (patchRequest: PatchRequestArtifact): string[] =>
  unique(patchRequest.must_fix.flatMap((item) => item.target_check_ids).filter(Boolean));

const patchObjective = (targetCheckIds: readonly string[]): string => {
  if (
    targetCheckIds.length === 1 &&
...
export const buildSyntheticPatchCarryForwardAgreement = (input: {
  ...
  generator_must_deliver:
    input.previousPatchRequest.must_fix.length > 0
  ...
  evaluator_must_verify:
    input.contractArtifact.acceptance_checks.length > 0
      ? input.contractArtifact.acceptance_checks.map(
          (checkId) => `Verify carried patch target '${checkId}'.`
        )
