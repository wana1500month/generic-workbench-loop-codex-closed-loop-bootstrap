...
import type {
  AdapterMigrationAuthoringResponseArtifact,
...
const stringList = (value: unknown, limit = 16): string[] =>
  Array.isArray(value)
    ? [...new Set(
        value
...
const expectedIdentityLines = (
  identity: AdapterMigrationIdentitySnapshot
): string[] =>
  [
...
const authoringModeForProposal = (
  proposal: AdapterMigrationProposal
): AdapterMigrationAuthoringTaskArtifact["authoring_mode"] =>
  proposal.same_run_eligible && proposal.apply_mode === "same_run_in_place"
    ? "same_run_apply"
    : "proposal_bundle";

const allowedPathsForProposal = (
  proposal: AdapterMigrationProposal,
...
const promptText = (input: {
  task: AdapterMigrationAuthoringTaskArtifact;
  proposal: AdapterMigrationProposal;
}): string =>
  [
    ...
    "",
    "## Approval semantics",
    ...Object.entries(approvalSemanticsForAdapterMigrationProposal(input.proposal)).map(
...
export const writeAdapterMigrationAuthoringTask = async (input: {
  ...
  const task: AdapterMigrationAuthoringTaskArtifact = {
    run_id: input.runId,
    round: input.round,
  ...
  await Promise.all([
    ...
    writeText(
      input.artifacts.adapter_migration_authoring_prompt_path,
      promptText({
        task,
...
const parseJsonResponse = async <T>(path: string): Promise<T | undefined> => {
...
export const readAdapterMigrationAuthoringResponse = async (
  path: string,
  expectedCheckpointId?: string
): Promise<AdapterMigrationAuthoringResponseArtifact | undefined> => {
  const parsed =
    (await parseJsonResponse<AdapterMigrationAuthoringResponseArtifact>(path)) ??
    (await loadJsonIfExists<AdapterMigrationAuthoringResponseArtifact>(path));
  if (!parsed || typeof parsed !== "object") {
    return undefined;
  }
  if (
    parsed.status !== "authored" &&
