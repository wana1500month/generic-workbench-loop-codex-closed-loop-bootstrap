...
import {
  ...
  inferProjectKindFromText,
  type EvaluationStrictnessLevel,
...
const parseStrictnessLevel = (
  value: string
): EvaluationStrictnessLevel | undefined => {
  const match =
    value.match(/(?:strictness|엄격도|까다롭|깐깐|출시\s*리뷰)[^\d]{0,12}([1-5])/iu) ??
...
const normalizeMetricId = (label: string): string =>
  `custom.${label
    .normalize("NFKC")
    .toLowerCase()
...
const scorelessQualityMetricDefinitions: readonly {
  metric_id: string;
  label: string;
  description: string;
  weight: number;
...
}[] = [
  {
    ...
    label: "No noisy text",
    description:
      "Unnecessary helper copy, placeholder text, dummy text, and excessive explanation should be penalized.",
    weight: 2,
    patterns: [
  ...
  },
  {
    ...
    label: "App-like feel",
    description:
      "The surface should feel like a focused application, not a landing page, generic web page, admin template, or scaffold.",
    weight: 1.5,
    patterns: [
  ...
  },
  {
    ...
    description:
      "Spacing, alignment, hierarchy, and visual restraint should feel clean and deliberate.",
    weight: 2,
    patterns: [
  ...
  },
  {
    ...
    description:
      "Error messages should be understandable and actionable for the intended user.",
    weight: 1,
    patterns: [
  ...
  },
  {
    ...
    description:
      "Output format and result structure should stay stable and reproducible across runs.",
    weight: 1,
    patterns: [
...
const metricRequiredByText = (value: string): boolean =>
  !/optional|\uC120\uD0DD|\uCC38\uACE0/u.test(value);

const parseExplicitCustomQualityMetrics = (
  value: string
): SessionCustomQualityMetric[] | undefined => {
  const workflowMetricLabelPattern =
    /^(?:workflow\s+checks?|workflow\s+probes?|workflows?|\uC6CC\uD06C\uD50C\uB85C\s*\uCCB4\uD06C|\uC6CC\uD06C\uD50C\uB85C)$/iu;
  const metrics = splitAnswerLines(value)
    .map((line): SessionCustomQualityMetric | undefined => {
      const match =
        ...
        line.match(
          /^([^:：>=]+?)\s*(?:>=|이상|최소)\s*([0-9]+(?:\.[0-9]+)?)\s*(?:점|\/\s*10)?\.?\s*(.*)$/u
        );
      if (!match?.[1] || !match[2]) {
        return undefined;
      }
      ...
      const description =
        normalizeInlineValue(match[3] ?? "") || `${label} quality dimension.`;
      return {
        ...
        weight: /디자인|깔끔|텍스트|문구|앱스러|visual|design|copy/u.test(label)
          ? 2
          : 1
      } satisfies SessionCustomQualityMetric;
    })
    .filter((metric): metric is SessionCustomQualityMetric => metric !== undefined);

  return metrics.length > 0 ? metrics : undefined;
};

const inferScorelessCustomQualityMetricLine = (
  line: string,
...
const parseCustomQualityMetrics = (
  value: string,
  strictnessLevel: EvaluationStrictnessLevel
): SessionCustomQualityMetric[] | undefined => {
  const byId = new Map<string, SessionCustomQualityMetric>();
...
const inferProjectKindCandidate = (value: string): ProjectKind | undefined => {
  const projectKind = inferProjectKindFromText(value);
  return projectKind === "generic" ? undefined : projectKind;
};

const evidenceSurfacesForCandidate = (
  projectKind: ProjectKind | undefined,
  explicitSurfaces: VerificationSurface[] | undefined
...
const verificationSurfaceClausePattern =
  /(?:verification\s*(?:surfaces?|methods?|evidence)|evidence\s*(?:surfaces?|with|via)|proof\s*(?:with|via)|verify\s*(?:only\s*)?(?:with|in|via)|browser\s+verification|screen\s+verification|\uAC80\uC99D\s*(?:\uBC29\uC2DD|\uC218\uB2E8|\uBC29\uBC95|\uC99D\uAC70|\uD45C\uBA74)|\uD654\uBA74\uC73C\uB85C\s*\uAC80\uC99D|\uBE0C\uB77C\uC6B0\uC800\uB85C\s*\uAC80\uC99D|API\uB85C\s*\uAC80\uC99D|\uD14C\uC2A4\uD2B8\s*\uBA85\uB839\uC73C\uB85C\s*\uAC80\uC99D)/iu;

const explicitVerificationSurfaceAnswerText = (
  message: string,
  input: { adapterQuestionPair: boolean }
): string | undefined => {
  ...
  questionIds.forEach((fieldId, index) => {
    ...
    switch (fieldId) {
      ...
      case "workflow_checks": {
        const explicitSurfaceText = explicitVerificationSurfaceAnswerText(
          message,
          { adapterQuestionPair: false }
        );
  ...
  const candidateVerificationSurfaces =
    explicitVerificationSurfaces.length
        ...
        : adapterQuestionPair && intakeResult.extracted_verification_surfaces?.length
          ...
          : undefined;
  const workflowChecksFromMessage = parseWorkflowChecksAnswer(
    message,
    candidateVerificationSurfaces?.[0] ??
      input.existingIntake?.verification_surfaces?.[0] ??
      "browser"
  );
  const candidateWorkflowChecks =
...
const applyEvidenceSurfaces = (
  target: SessionIntakeSnapshot,
  candidate: EvidenceSurface[] | undefined,
  conflicts: FrontDoorSessionConflict[],
  options: { replace?: boolean } = {}
): void => {
  if (!candidate?.length) {
...
const applyCustomQualityMetrics = (
  target: SessionIntakeSnapshot,
  candidate: SessionCustomQualityMetric[] | undefined,
  conflicts: FrontDoorSessionConflict[],
  options: { replace?: boolean } = {}
): void => {
  if (!candidate?.length) {
  ...
  const byId = new Map(
    target.custom_quality_metrics.map((metric) => [metric.metric_id, metric])
  );
...
export const mergeFrontDoorSessionTurn = (input: {
  ...
  applyVerificationSurfaces(nextIntake, candidates.verification_surfaces, conflicts, {
    replace:
      replaceFields.has("verification_surfaces") ||
  ...
  applyCustomQualityMetrics(
    nextIntake,
    candidates.custom_quality_metrics,
    conflicts,
    {
  ...
  if (nextIntake.project_kind && !nextIntake.evidence_surfaces?.length) {
    nextIntake.evidence_surfaces = evidenceSurfacesForProjectKind(
      nextIntake.project_kind
    );
  }
