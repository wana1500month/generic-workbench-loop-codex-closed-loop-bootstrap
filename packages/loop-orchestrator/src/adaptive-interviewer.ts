...
export type AdaptiveQuestionField =
...
const workflowQuestionFor = (projectKind: ProjectKind): string => {
  switch (projectKind) {
    case "cli_tool":
...
const qualityQuestionFor = (projectKind: ProjectKind): string => {
  switch (projectKind) {
    case "browser_ui":
    case "mobile_ui":
...
const failureQuestionFor = (projectKind: ProjectKind): string => {
  switch (projectKind) {
    case "cli_tool":
...
export const buildAdaptiveQuestionSet = (input: {
  ...
  const candidates: AdaptiveQuestionCandidate[] = [
    {
      ...
      why_it_matters:
        "The evaluator needs an evidence surface before it can judge real behavior instead of prose.",
    ...
    },
    {
      ...
      why_it_matters:
        "A loop cannot prove completion without at least one concrete action and expected result.",
    ...
    },
    {
      ...
      why_it_matters:
        "User-authored quality dimensions decide whether a high total score is actually acceptable.",
    ...
    },
    {
      ...
      why_it_matters:
        "The highest-risk failure case helps the harness avoid passing a shallow happy-path implementation.",
  ...
  return {
    ...
    ) as Partial<Record<AdaptiveQuestionField, AdaptiveQuestionCandidate>>
  };
};
