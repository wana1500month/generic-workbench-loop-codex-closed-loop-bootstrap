...
import {
  crashAfterCheckpointEnabled,
  ensureJsonFile,
  isImproved,
  roundDirectoryFor,
  writeRoundSummary
} from "./loop/round-files.js";
import {
  buildRuntimeEvent,
    ...
    if (phaseCompletedAtOrBeyond(resumedRoundPhase, "evaluation")) {
      ...
      if (evaluationPolicy) {
        ...
        if (roundScorecard.blocking_reasons.length > 0) {
          const scorecardGapDetails = roundScorecard.blocking_reasons.map(
            (reason) =>
          ...
          evalReport = {
            ...
            blockers: unique([
              ...evalReport.blockers,
              ...roundScorecard.blocking_reasons.map(
                (reason) =>
                  `Required evaluation dimension failed: ${reason.dimension_id}`
              )
            ]),
            next_actions: unique([
