...
const normalizedComparisonValue = (value?: string | boolean | number): string => {
...
export const resumeIdentityArtifactPath = (runDirectory: string): string =>
  join(runDirectory, "resume-identity.json");

export const loadResumeIdentityArtifact = async (
  runDirectory: string
): Promise<ResumeIdentityState | undefined> => {
  ...
  return loadJson<ResumeIdentityState>(artifactPath);
};
...
export const buildResumeIdentityState = async (input: {
  ...
  return {
    ...
    evaluator_bundle_attached: Boolean(
      normalized(input.evaluatorProfilePath) || evaluatorBundleSha
    ),
...
export const resumeIdentityFingerprint = (identity: ResumeIdentityState): string =>
  JSON.stringify({
...
export const summaryResumeIdentity = (
  summary?: LoopRunSummary
): ResumeIdentityState => ({
  resume_identity_version: 1,
  adapter_attached:
    summary?.adapter_attached ??
    Boolean(summary?.adapter_contract_path || summary?.adapter_contract_sha256),
  evaluator_bundle_attached: Boolean(
    summary?.evaluator_profile_path || summary?.evaluator_bundle_sha256
  ),
...
export const compareResumeIdentity = (input: {
  ...
  const compareField = (
    ...
    previousValue: string | boolean | number | undefined,
    options?: {
      requirePreviousPresenceInLegacyMode?: boolean;
    }
  ): void => {
    ...
    if (currentNormalized !== previousNormalized) {
      mismatches.push(
        `${label} changed from '${previousNormalized}' to '${currentNormalized}'.`
      );
    }
  };
  ...
  compareField(
    "adapter attached",
    input.current.adapter_attached,
    input.previous.adapter_attached
  );
  compareField(
    "adapter contract path",
    input.current.adapter_contract_path,
    input.previous.adapter_contract_path
  );
  compareField(
    "adapter contract fingerprint",
    input.current.adapter_contract_sha256,
    input.previous.adapter_contract_sha256
  );
  compareField(
    "evaluator bundle attached",
    input.current.evaluator_bundle_attached,
    input.previous.evaluator_bundle_attached
  );
  compareField(
    "evaluator bundle path",
    input.current.evaluator_profile_path,
    input.previous.evaluator_profile_path
  );
  compareField(
    "evaluator bundle fingerprint",
    input.current.evaluator_bundle_sha256,
    input.previous.evaluator_bundle_sha256
  );
  compareField(
    "rubric fingerprint",
    input.current.rubric_sha256,
    input.previous.rubric_sha256
  );
  compareField(
    "executor mode",
  ...
  compareField(
    "target family",
    input.current.target_family,
    input.previous.target_family,
    { requirePreviousPresenceInLegacyMode: true }
  );
  compareField(
    "validation lane",
    input.current.validation_lane,
    input.previous.validation_lane,
    { requirePreviousPresenceInLegacyMode: true }
  );
