...
const isPassingCheck = (result: RoundCheckResult): boolean =>
  result.status === "pass";
...
export const buildCarryForwardGateArtifact = (input: {
  ...
  const actionableTargetCheckIds = previousPatchTargetCheckIds.filter(
    (checkId) => !nonCarryForwardDerivedChecks.has(checkId)
  );
  const checkLookup = new Map(
    input.evalReport.check_results.map((result) => [result.check_id, result])
  );
  ...
  const missingTargetCheckIds = actionableTargetCheckIds.filter(
    (checkId) => !checkLookup.has(checkId)
  );
  const addressed =
    previousPatchTargetCheckIds.length === 0 ||
    input.previousPatchRequestAddressed;
  const resolved =
    actionableTargetCheckIds.length === 0
      ? addressed
      : addressed &&
