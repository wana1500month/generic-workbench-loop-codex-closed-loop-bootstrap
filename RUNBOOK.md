...
- `npm run validate:fast`: short deterministic commit gate for front-door, transport, security, score-policy, readiness doctor, prepare, continue, and durable-memory checks. Stateful batch suites such as `fast` and `product-front-door` isolate validator run/session state under a temporary `.tmp/validation-batch-<suite>-*` root unless explicit `HARNESS_RUNS_DIRECTORY` and `HARNESS_FRONT_DOOR_SESSIONS_DIRECTORY` values are already supplied.
