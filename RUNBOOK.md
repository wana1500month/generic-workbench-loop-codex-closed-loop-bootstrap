...
- `npm run validate:quick`: short deterministic front-door and productization gate for intent, intake, discovery, readiness, evaluation policy, scorecard output, non-web adapter-plan defaults, project-kind fixtures, Korean ambiguous document handling, Korean product phrasing variants, and Korean non-product rejection. Use this for fast local iteration before the heavier loop suites.
...
- Stateful batch suites such as `quick`, `fast`, and `product-front-door` isolate validator run/session state under a temporary `.tmp/validation-batch-<suite>-*` root unless explicit `HARNESS_RUNS_DIRECTORY` and `HARNESS_FRONT_DOOR_SESSIONS_DIRECTORY` values are already supplied.
...
- `npm run validate:korean-ambiguous-document-request`, `npm run validate:korean-product-kind-fixtures`, `npm run validate:korean-product-phrasing-variants`, `npm run validate:korean-document-artifact-detection`, `npm run validate:korean-data-pipeline-detection`, and `npm run validate:korean-non-product-rejection`: focused gates proving ambiguous Korean document requests ask a clarification question, Korean generator/tool/builder/template wording such as document generators, report generators, guide templates, pipeline builders, and CSV converters routes to `product_build`, while document summary/analysis/translation/review requests remain non-product.
...
- `npm run validate:core`: deterministic adapter-free integration gate; it includes `validate:quality-lift` so generated evaluator bundles cannot lower the selected base family release-gate floor.
