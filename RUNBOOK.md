...
- `.agents/skills/*/scripts/*.mjs`: plugin-facing guidance treats missing `packages/loop-orchestrator/dist` as a source archive signal first. Prefer installing `.tmp/release/generic-codex-workbench.zip`; run `bash ./init.sh` only when the operator accepts local npm bootstrap, then fall back to direct `bash ./init.sh` or `npm ci && npm run build` recovery if bootstrap fails.
