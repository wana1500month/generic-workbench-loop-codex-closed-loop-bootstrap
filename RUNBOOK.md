...
- `npm run release:zip`: builds the workspace, packages `.tmp/release/generic-codex-workbench.zip`, and validates that the installable ZIP includes `packages/loop-orchestrator/dist` while excluding `node_modules`, `.tmp`, and persisted run artifacts
