...
- `npm run loop:discover -- --message "<answer>" --json`: stateful file-backed product discovery front door that accumulates intake answers per thread under `evals/front-door-sessions/`, remembers the last question ids for terse answer merging, restores the latest snapshot without depending on chat history, and becomes immutable after prepare. It returns `ask_conflict_resolution` instead of `ready_for_prepare` while unresolved target-family or scope conflicts remain. Pass `--thread-id` only when a stable thread id is available; otherwise the CLI falls back to `CODEX_THREAD_ID`, `HARNESS_FRONT_DOOR_THREAD_ID`, then `local-codex-thread`.
- `npm run loop:prepare -- --front-door-session <evals/front-door-sessions/session-thread-id.json> --json`: consumes a persisted discovery session only after it reaches `ready_for_prepare`, materializes `intake.json` into both the prepare root and target root, writes the session-owned `build-brief`, `run-contract`, `operator-surface`, `session-status`, `session-stream`, `runtime/readiness-report.json`, `runtime/readiness-report.md`, `evaluation-policy.generated.json`, `evaluation-policy.generated.md`, and execution-plan artifacts. If readiness doctor finds blockers, the session stops at `prepared_with_blockers`, does not write `runtime/ready-to-start.md`, and does not index the run under `evals/runs/ready-to-start/`; the report lists blocker code, human explanation, why it matters, how to fix it, owner, and next action. Only blocker-free sessions write `runtime/ready-to-start.md` and leave the run at the explicit `ready_to_start` gate. For product-family sessions it persists generated adapter artifacts under `evals/runs/<run-id>/generated-adapter/`, including `generated-adapter/evaluation-policy.generated.json`, records the prepared adapter/rubric/evaluator bundle identity into the prepare surfaces, and indexes ready sessions by run and thread.
...
- once a prepared same-thread session starts, its persisted session status now moves to `running`, its ready index entries are consumed, started runs with `summary.json` are excluded from `ready_to_start` discovery, and ambiguous unbound starts fail closed instead of consuming the wrong prepared run
...
- `docs/TARGET_FAMILIES_AND_EVIDENCE_SURFACES.md`: target-family / project-kind mapping for evaluator bundles, evidence surfaces, and command-first runtime expectations
- `docs/VALIDATION_RELIABILITY.md`: deterministic vs live Codex validation boundaries plus repeat and batch-isolation rules for front-door sessions
...
- `npm run release:package`: package the current built workspace into `.tmp/release/generic-codex-workbench-CODEX-APP-INSTALL.zip`.
- `npm run release:zip`: build the workspace, then run `release:package`. It creates the installable ZIP but does not run release validators.
- `npm run validate:release`: build, run `validate:quality-lift`, package the ZIP, validate ZIP contents, and prove release-root product discovery -> prepare -> `loop:start:codex` can consume the prepared run without `npm ci`.
...
Deterministic validation gates run from repository fixtures and generated harness artifacts. They should not require a live Codex account, browser installation, external adapter, or networked product runtime unless the command name explicitly says so. Use these gates for normal local commits and release packaging.

Live Codex validation gates exercise trusted external surfaces such as the real `codex` binary, authenticated `codex exec`, App Server, browser realism preflight, or external adapters. They may return `environment_blocked` on machines without those dependencies; that is different from deterministic gate failure and should be documented as an environment preflight result.

- `npm run validate:quick`: short deterministic front-door and productization gate for intent, intake, discovery, readiness, evaluation policy, scorecard output, non-web adapter-plan defaults, project-kind fixtures, Korean ambiguous document handling/follow-up, Korean product phrasing variants, and Korean non-product rejection. Use this for fast local iteration before the heavier loop suites.
- `npm run validate:fast`: medium deterministic commit gate that extends `validate:quick` with loop scorecard/default-policy, adaptive intake, non-web target, CLI product detection, fast-exit, and prepare coverage. The slow lifecycle/full matrix lanes live in `validate:core-long`.
- Stateful batch suites such as `quick`, `fast`, `product-front-door`, and `productization` isolate validator run/session state under a temporary `.tmp/validation-batch-<suite>-*` root unless explicit `HARNESS_RUNS_DIRECTORY` and `HARNESS_FRONT_DOOR_SESSIONS_DIRECTORY` values are already supplied.
- `npm run validate:front-door-session-repeat`: focused reliability gate that runs the stateful front-door session validator three times with isolated temp roots to catch session directory, thread id, or environment leaks.
- `npm run validate:validation-batch-isolation`: focused reliability gate that compares normalized standalone output with the same validator executed through `run-validation-batch.mjs isolation-smoke`.
...
- `npm run validate:library-front-door-questions`, `npm run validate:agent-workflow-front-door-questions`, `npm run validate:document-artifact-front-door-questions`, and `npm run validate:data-pipeline-front-door-questions`: focused gates proving first-turn product questions match the inferred project kind instead of falling back to browser/UI assumptions.
- `npm run validate:project-kind-fixtures`: fixture gate covering detection, first-turn questions, adapter plan, readiness, evaluation policy, and default scorecard behavior for CLI tools, libraries, data pipelines, agent workflows, document artifacts, automation, browser UI, and API services.
- `npm run validate:korean-ambiguous-document-request`, `npm run validate:korean-ambiguous-document-followup`, `npm run validate:korean-product-kind-fixtures`, `npm run validate:korean-product-phrasing-variants`, `npm run validate:korean-document-artifact-detection`, `npm run validate:korean-data-pipeline-detection`, and `npm run validate:korean-non-product-rejection`: focused gates proving ambiguous Korean document requests ask a clarification question, clarification follow-up can resolve into `document_artifact` / `command-artifact`, Korean generator/tool/builder/template wording such as document generators, report generators, guide templates, pipeline builders, and CSV converters routes to `product_build`, while document summary/analysis/translation/review requests remain non-product.
...
- `npm run validate:product-front-door`: focused front-door/product suite covering intent, intake, stateful discovery, CLI/non-web routing, project-kind fixtures, ambiguous Korean document follow-up, Korean product phrasing, and Korean non-product rejection. It does not build or validate the release ZIP, run prepare consumption, or execute generated adapter run-local checks.
- `npm run validate:productization`: focused productization suite covering readiness doctor, evaluation policy, strictness, scorecards, adaptive intake, non-web readiness, and non-web loop closure. Front-door fixture breadth lives in `validate:product-front-door` and release packaging lives in `validate:release`.
...
- `npm run validate:browser-only-no-api-probes` / `npm run validate:semantic-workflow-selectors`: generated contract regression checks proving browser-only profiles exclude API/draft/error probes and prefer semantic workflow selectors with legacy fallbacks.
...
- `npm run validate:core`: deterministic adapter-free integration gate for front-door repeatability, batch isolation, transport/security policy, score/strictness policy, prepare, foreground ownership, and baseline validity.
- `npm run validate:core-long`: longer deterministic integration sweep; it extends `validate:core` with lifecycle matrix, quality-lift, loop-continuation, and durable-memory coverage.
- `npm run validate:smoke-clean`: hydrates `.tmp/semantic-validation` from `scripts/testing/fixtures/semantic-validation`, clears only fixture runtime state, and proves smoke is self-contained.
- `npm run validate:semantic-target-server-cleanup`: runs `validate:quality-lift`, `validate:productization`, and `validate:smoke-clean`, then fails if a repo-owned semantic `target-server.cjs` Node process is still alive.
- `npm run validate:release`: builds and validates the installable Codex app ZIP, including the release-path `validate:quality-lift` guard. This is the ZIP validation path; ordinary product front-door batches and `release:zip` packaging must not repeat release validation.
- `npm run validate:source-release-identity`: proves the source checkout carries `SOURCE_ARCHIVE_NOT_CODEX_APP_INSTALL.md`, does not carry install-only `CODEX_APP_INSTALL.md` or `release-manifest.json`, and that release packaging creates the install-only markers while excluding the source-only marker.
- `npm run validate:source-archive-repro`: stages a source archive candidate without `.tmp/`, compiled dist, or `*.tsbuildinfo`, then force-builds and runs `npm ci`, `npm run build`, `npm test`, `npm run validate:smoke-clean`, and `npm run validate:release`.
- `npm test`: release-blocking local test path; it runs `validate:process` before `validate:core`.
- `npm run validate:nightly`: `validate:core-long` plus smoke-clean lane. Use `validate:semantic-target-server-cleanup` as the explicit process-leak follow-up when changing semantic fixtures or validation cleanup.
- `npm run validate:release-gate`: stricter release gate that first proves source/install identity and source-archive reproducibility, then builds and validates the install ZIP and release-start path. CLI live and heavy nightly lanes are still intentionally separate from app installation.
...
- Validators and trusted harness wrappers may set `HARNESS_RUNS_DIRECTORY` to isolate temporary run artifacts from `evals/runs`; normal operator runs default to `evals/runs`.
...
- `npm run loop:start:codex` is the Codex-owned current-thread start surface. When a thread-bound `ready_to_start` session prepared by `loop:prepare` or `app-builder-loop` exists, `loop:start:codex` now consumes that prepared run instead of allocating a fresh one. Use `npm run loop:start:codex -- --json` when a same-thread worker needs machine-readable continuation state immediately after start. If a Codex app skill is running without a stable `CODEX_THREAD_ID`, use `npm run loop:start:codex -- --json --run-id <run-id> --codex-app-foreground`; this assumed-foreground path is only allowed with an explicit run id. Use `npm run loop:resume -- --run-dir <evals/runs/run-###> --json` for machine-readable foreground re-entry, and use `npm run loop:continue -- --run-dir <evals/runs/run-###> --json` for the same-thread autocontinue dispatcher. Use `npm run loop:start:bg -- ...` or the deprecated `npm run loop:run -- ...` only when the operator explicitly wants detached background supervision.
- Codex CLI resume compatibility is intentionally version-aware: local `codex-cli 0.130.0-alpha.5` supports `codex exec resume` with `-c`, `--json`, and `--output-last-message`, but not top-level `--profile`, `--sandbox`, `--add-dir`, or `--output-schema`. The runtime therefore reapplies sandbox mode on resume through `-c sandbox_mode="..."` and keeps unsupported resume flags out of the invocation.
...
- The same session-preparation pass should also write `runtime/open-questions.json`, `runtime/session-status.json`, `runtime/session-status-events.jsonl`, `runtime/session-stream.json`, `runtime/readiness-report.json`, `runtime/readiness-report.md`, and `docs/EXECUTION_PLAN.md` for the active run, and refresh them when round feedback, human steering, readiness blockers, or external blockers change the active session context.
...
- Generated-local runtime-surface drift may now auto-apply inside the recontract round when the repair stays within the generated adapter contract and runtime surface, including legacy `.generated/codex-adapter/*` paths and run-local `generated-adapter/codex-adapter/*` paths; that path writes `adapter-migration-proposal.json`, `adapter-migration-applied.json`, and an authorized `resume-migration.json` before the same run continues with the migrated adapter identity.
...
- Browser baseline and core browser probes first run a child-process `playwright-core` import probe before importing Playwright in the controller process. If a local Node/Playwright/native combination crashes during import, the harness records blocked or environment-blocked browser evidence instead of terminating the controller.
