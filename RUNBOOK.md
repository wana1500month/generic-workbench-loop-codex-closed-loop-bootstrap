...
- `npm run release:zip`: builds the workspace, runs `validate:quality-lift`, packages `.tmp/release/generic-codex-workbench-CODEX-APP-INSTALL.zip`, validates that the installable ZIP includes `packages/loop-orchestrator/dist`, `CODEX_APP_INSTALL.md`, and `release-manifest.json` while excluding `node_modules`, `.tmp`, `SOURCE_ARCHIVE_NOT_CODEX_APP_INSTALL.md`, persisted run artifacts, and front-door session state, then proves release-root product discovery -> prepare -> `loop:start:codex` can consume the prepared run without `npm ci`
...
- `npm run validate:fast`: medium deterministic commit gate that extends `validate:quick` with loop scorecard/default-policy, adaptive intake, non-web target, CLI product detection, fast-exit, and prepare coverage. The slow `loop-continue` and durable-memory lanes live in `validate:core`.
...
- `npm run validate:core`: deterministic adapter-free integration gate; it includes front-door repeatability, batch isolation, and `validate:quality-lift` so generated evaluator bundles cannot lower the selected base family release-gate floor.
...
- `npm run validate:release`: builds and validates the installable Codex app ZIP, including the release-path `validate:quality-lift` guard. This is the ZIP validation path; ordinary product front-door batches must not repeat release packaging.
