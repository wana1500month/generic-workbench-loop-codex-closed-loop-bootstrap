...
- `npm run validate:quick`: short deterministic front-door and productization gate for intent, intake, discovery, readiness, evaluation policy, scorecard output, non-web adapter-plan defaults, project-kind fixtures, Korean product phrasing variants, and Korean non-product rejection. Use this for fast local iteration before the heavier loop suites.
...
- `npm run validate:korean-product-kind-fixtures`, `npm run validate:korean-product-phrasing-variants`, `npm run validate:korean-document-artifact-detection`, `npm run validate:korean-data-pipeline-detection`, and `npm run validate:korean-non-product-rejection`: focused gates proving Korean generator/tool/builder/template wording such as document generators, report generators, guide templates, pipeline builders, and CSV converters routes to `product_build`, while document summary/analysis/translation/review/write requests remain non-product.
