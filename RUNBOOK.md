...
- `npm run validate:quick`: short deterministic front-door and productization gate for intent, intake, discovery, readiness, evaluation policy, scorecard output, non-web adapter-plan defaults, project-kind fixtures, and Korean non-product rejection. Use this for fast local iteration before the heavier loop suites.
...
- `npm run validate:korean-product-kind-fixtures`, `npm run validate:korean-document-artifact-detection`, `npm run validate:korean-data-pipeline-detection`, and `npm run validate:korean-non-product-rejection`: focused gates proving Korean generator/tool wording such as document generators and pipeline generators routes to `product_build`, while document summary/analysis/translation/review requests remain non-product.
