...
- `npm run loop:discover -- --thread-id <thread-id> --message "<answer>" --json`: file-backed product discovery session that accumulates intake answers per thread under `evals/front-door-sessions/` and restores the latest snapshot without depending on chat history
